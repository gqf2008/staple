...
- [x] F3 侧栏（issue #237 + #244）：默认 240px + 折叠 rail 64px（#sidebar-toggle + localStorage，折叠后隐藏文本残片）；拖拽调整宽度已实现（#sidebar-resizer，208–420px 夹紧 + localStorage 持久化 + 键盘 Arrow/Home/End + aria-valuenow）；≤48rem 窄屏为 off-canvas drawer 近似（issue #248：汉堡开关 + 遮罩 + Esc/遮罩点击关闭 + 状态持久化 + 断点切换实时重同步；与上游 drawer 的差异仅为 SSR 渲染方式）。根页 `.inline-form` 窄屏溢出已修复（issue #246）。
