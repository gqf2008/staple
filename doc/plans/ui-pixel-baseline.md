...
> **更新（2026-08-08，issue #252）**：组件级视觉对齐——Button 字重 500、Badge 胶囊（22px，`--radius-full` + `space-0-5 space-2`）、命令面板（448px/`radius-lg`/`shadow-md`/输入 h-12/项 `space-3 space-2`）、看板卡（`space-2-5`/`radius-lg`/hover `shadow-sm`）；列表行卡保留为 B4 卡片式近似（登记）。
...
- [x] F3 侧栏（issue #237 + #244）：默认 240px + 折叠 rail 64px（#sidebar-toggle + localStorage，折叠后隐藏文本残片）；拖拽调整宽度已实现（#sidebar-resizer，208–420px 夹紧 + localStorage 持久化 + 键盘 Arrow/Home/End + aria-valuenow）；≤48rem 窄屏为 off-canvas drawer 近似（issue #248：汉堡开关 + 遮罩 + Esc/遮罩点击关闭 + 状态持久化 + 断点切换重同步；差异为 SSR 渲染方式 + 无手势滑动 + 无 body 滚动锁，已登记）。根页 `.inline-form` 窄屏溢出已修复（issue #246）。
