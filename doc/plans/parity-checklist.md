...
- 2026-08-06：登记上游 8-03→8-04 增量（`sync/upstream` 落后于 `upstream/master`，手动触发镜像同步）：
  - **attention/决策桌 triage**（#10785）：`deskBadgeCount`（今天新增/到期，分页前计算）、`blockedTaskCount`、blocker 树 `blockingTreeLive`/`terminalBlockerIssueId`、按阻塞权重排序、pending request_confirmation 折叠 → issue #204（阶段 A1 ✅ 核心 feed；阶段 A2 ✅ dismiss/snooze + includeDismissed/activitySince/Until/all/queue + failed_run/decision kinds + queue 富集；阶段 B ✅ blocker 多级树遍历（depth≤8/nodes≤200）+ 权重排序；阶段 C UI 待续）
