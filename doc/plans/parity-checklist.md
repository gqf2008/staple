...
| 指令文件体系（AGENTS.md/SOUL.md/HEARTBEAT.md 挂载） | 上游 onboarding-assets 属指令系统（agent instructions 路由 + 文档挂载），独立功能面；install API/UI 也按 role 初始化默认指令（#207） | ✅ 完成（issue #200） |
...
- 2026-08-06：登记上游 8-03→8-04 增量（`sync/upstream` 落后于 `upstream/master`，手动触发镜像同步）：
  - **attention/决策桌 triage**（#10785）：`deskBadgeCount`（今天新增/到期，分页前计算）、`blockedTaskCount`、blocker 树 `blockingTreeLive`/`terminalBlockerIssueId`、按阻塞权重排序、pending request_confirmation 折叠 → issue #204 ✅ 全部完成（阶段 A1 核心 feed；A2 dismiss/snooze + 参数 + failed_run/decision kinds + queue 富集；B blocker 多级树 + 权重排序；C WhatNeedsMe 页渲染 attention feed + deskBadgeCount badge + dismiss 表单 + 导航链接）
  - **共享工作区并发**：`sharedWorkspaceConcurrency`（auto/serialize/allow，project policy + issue settings + 执行目标/心跳序列化）→ issue #206 ✅ 全部完成（阶段 A 数据结构+策略解析；阶段 B shared_workspace_key 读写 + workspace_operations 忙检测 + start_run 409 延迟 + scheduler 有界重试 ≤3）
