...
export function ExecutionWorkspaceCloseDialog({
  ...
  const actionLabel = currentStatus === "cleanup_failed" ? t("components.executionWorkspaceCloseDialog.retryClose", { defaultValue: "Retry close" }) : t("components.executionWorkspaceCloseDialog.closeWorkspace", { defaultValue: "Close workspace" });
  ...
  const closeWorkspace = useMutation({
    ...
    onSuccess: (workspace) => {
      ...
      pushToast({
        title: currentStatus === "cleanup_failed" ? t("components.executionWorkspaceCloseDialog.closeRetried", { defaultValue: "Workspace close retried" }) : t("components.executionWorkspaceCloseDialog.workspaceClosed", { defaultValue: "Workspace closed" }),
        tone: "success",
      });
      onOpenChange(false);
      onClosed?.(workspace);
    ...
    onError: (error) => {
      pushToast({
        title: t("components.executionWorkspaceCloseDialog.closeFailed", { defaultValue: "Failed to close workspace" }),
        body: error instanceof Error ? error.message : t("components.executionWorkspaceCloseDialog.unknownError", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  });
          ...
          <div className="space-y-4">
            <div className={`rounded-xl border px-4 py-3 text-sm ${readinessTone(readiness.state)}`}>
              <div className="font-medium">
                {readiness.state === "blocked"
                  ? t("components.executionWorkspaceCloseDialog.closeBlocked", { defaultValue: "Close is blocked" })
                  : readiness.state === "ready_with_warnings"
                    ? t("components.executionWorkspaceCloseDialog.closeWithWarnings", { defaultValue: "Close is allowed with warnings" })
                    : t("components.executionWorkspaceCloseDialog.closeReady", { defaultValue: "Close is ready" })}
              </div>
                ...
                {readiness.isSharedWorkspace
                  ...
                  : readiness.git?.workspacePath && readiness.git.repoRoot && readiness.git.workspacePath !== readiness.git.repoRoot
                    ...
                    : readiness.isProjectPrimaryWorkspace
                      ? t("components.executionWorkspaceCloseDialog.primaryPathHint", { defaultValue: "This execution workspace currently points at the project's primary workspace path." })
                      : t("components.executionWorkspaceCloseDialog.disposableHint", { defaultValue: "This workspace is disposable and can be archived." })}
              </div>
            </div>

            {blockingIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("components.executionWorkspaceCloseDialog.blockingTasks", { defaultValue: "Blocking tasks" })}</h3>
            ...
            {readiness.blockingReasons.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("components.executionWorkspaceCloseDialog.blockingReasons", { defaultValue: "Blocking reasons" })}</h3>
            ...
            {readiness.warnings.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("components.executionWorkspaceCloseDialog.warnings", { defaultValue: "Warnings" })}</h3>
                <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            {readiness.git ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("components.executionWorkspaceCloseDialog.gitStatus", { defaultValue: "Git status" })}</h3>
                <div className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                  <div className="grid gap-2 sm:grid-cols-2">
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("components.executionWorkspaceCloseDialog.branch", { defaultValue: "Branch" })}</div>
                      <div className="font-mono text-xs">{readiness.git.branchName ?? t("components.executionWorkspaceCloseDialog.unknown", { defaultValue: "Unknown" })}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("components.executionWorkspaceCloseDialog.baseRef", { defaultValue: "Base ref" })}</div>
                      <div className="font-mono text-xs">{readiness.git.baseRef ?? t("components.executionWorkspaceCloseDialog.notSet", { defaultValue: "Not set" })}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("components.executionWorkspaceCloseDialog.mergedIntoBase", { defaultValue: "Merged into base" })}</div>
                      <div>{readiness.git.isMergedIntoBase == null ? t("components.executionWorkspaceCloseDialog.unknown", { defaultValue: "Unknown" }) : readiness.git.isMergedIntoBase ? t("components.executionWorkspaceCloseDialog.yes", { defaultValue: "Yes" }) : "No"}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("components.executionWorkspaceCloseDialog.aheadBehind", { defaultValue: "Ahead / behind" })}</div>
                      <div>
                        {(readiness.git.aheadCount ?? 0).toString()} / {(readiness.git.behindCount ?? 0).toString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("components.executionWorkspaceCloseDialog.dirtyTracked", { defaultValue: "Dirty tracked files" })}</div>
                      <div>{readiness.git.dirtyEntryCount}</div>
                    </div>
                    <div>
                      <div className="text-xs uppercase tracking-(--tracking-eyebrow) text-muted-foreground">{t("components.executionWorkspaceCloseDialog.untrackedFiles", { defaultValue: "Untracked files" })}</div>
            ...
            {otherLinkedIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("components.executionWorkspaceCloseDialog.otherLinkedTasks", { defaultValue: "Other linked tasks" })}</h3>
                <div className="space-y-2">
            ...
            {readiness.runtimeServices.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("components.executionWorkspaceCloseDialog.runtimeServices", { defaultValue: "Attached runtime services" })}</h3>
                <div className="space-y-2">
                  {readiness.runtimeServices.map((service) => (
                    <div key={service.id} className="rounded-xl border border-border bg-background px-4 py-3 text-sm">
                      ...
                      <div className="mt-1 break-words text-xs text-muted-foreground">
                        {service.url ?? service.command ?? service.cwd ?? t("components.executionWorkspaceCloseDialog.noDetails", { defaultValue: "No additional details" })}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section className="space-y-2">
              <h3 className="text-sm font-medium">{t("components.executionWorkspaceCloseDialog.cleanupActions", { defaultValue: "Cleanup actions" })}</h3>
              <div className="space-y-2">
