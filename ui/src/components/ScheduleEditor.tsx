...
const PRESETS: { value: SchedulePreset; label: string }[] = [
  { value: "every_minute", label: t("components.scheduleEditor.everyMinute", { defaultValue: "Every minute" }) },
  { value: "every_hour", label: t("components.scheduleEditor.everyHour", { defaultValue: "Every hour" }) },
  { value: "every_day", label: t("components.scheduleEditor.everyDay", { defaultValue: "Every day" }) },
  { value: "weekdays", label: t("components.scheduleEditor.weekdays", { defaultValue: "Weekdays" }) },
  { value: "weekly", label: t("components.scheduleEditor.weekly", { defaultValue: "Weekly" }) },
  { value: "monthly", label: t("components.scheduleEditor.monthly", { defaultValue: "Monthly" }) },
  { value: "custom", label: t("components.scheduleEditor.custom", { defaultValue: "Custom (cron)" }) },
];
...
const DAYS_OF_WEEK = [
  { value: "1", label: t("components.scheduleEditor.mon", { defaultValue: "Mon" }) },
  { value: "2", label: t("components.scheduleEditor.tue", { defaultValue: "Tue" }) },
  { value: "3", label: t("components.scheduleEditor.wed", { defaultValue: "Wed" }) },
  { value: "4", label: t("components.scheduleEditor.thu", { defaultValue: "Thu" }) },
  { value: "5", label: t("components.scheduleEditor.fri", { defaultValue: "Fri" }) },
  { value: "6", label: t("components.scheduleEditor.sat", { defaultValue: "Sat" }) },
  { value: "0", label: t("components.scheduleEditor.sun", { defaultValue: "Sun" }) },
];
...
function describeSchedule(cron: string): string {
  ...
  switch (preset) {
    case "every_minute":
      return t("components.scheduleEditor.everyMinute", { defaultValue: "Every minute" });
    case "every_hour":
    ...
    case "custom":
      return cron || t("components.scheduleEditor.noSchedule", { defaultValue: "No schedule set" });
  }
}
...
export function getScheduleCronValidation(cron: string): {
  ...
  if (!parseCronExpression(trimmed)) {
    return {
      valid: false,
      message: t("components.scheduleEditor.cronHint", { defaultValue: "Cron fields must use valid numbers, ranges, lists, wildcards, or steps." }),
      nextFires: [],
    };
  }

  const nextFires = nextCronFires(trimmed, 3, { timeZone: t("components.scheduleEditor.utc", { defaultValue: "UTC" }) });
  return {
    valid: true,
    message: nextFires.length > 0 ? t("components.scheduleEditor.validCron", { defaultValue: "Valid cron." }) : t("components.scheduleEditor.noFires", { defaultValue: "Valid cron, but no upcoming fires were found." }),
    nextFires,
  };
}
  ...
  return (
    <div className="space-y-3">
      <Select value={preset} onValueChange={(v) => handlePresetChange(v as SchedulePreset)}>
        <SelectTrigger className="w-full" aria-label={t("components.scheduleEditor.frequency", { defaultValue: "Schedule frequency" })}>
