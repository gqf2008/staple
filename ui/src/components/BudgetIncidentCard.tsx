...
function incidentStateLabel(incident: BudgetIncident) {
  if (incident.status === "resolved") return t("status.resolved", { defaultValue: "Resolved" });
  if (incident.status === "dismissed") return t("status.dismissed", { defaultValue: "Dismissed" });
  if (incident.approvalStatus === "revision_requested") return t("status.escalated", { defaultValue: "Escalated" });
  if (incident.approvalStatus === "pending") return t("status.pending", { defaultValue: "Pending approval" });
  return t("status.open", { defaultValue: "Open" });
}
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <div className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-red-700/90 dark:text-red-200/80">
                {t("common.hardStop", { scope: incident.scopeType })}
              </div>
            ...
            <CardDescription className="mt-1 text-red-900/75 dark:text-red-100/70">
              {t("common.spendVsLimit", { spent: formatCents(incident.amountObserved), limit: formatCents(incident.amountLimit) })}
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-4 px-5 pb-5 pt-0">
        <div className="flex items-start gap-2 rounded-xl border border-red-400/20 bg-red-500/10 px-3 py-2 text-sm text-red-950/90 dark:text-red-50/90">
          ...
          <div>
            {incident.scopeType === "project"
              ? t("common.projectPausedDesc", { defaultValue: "Project execution is paused. New work in this project will not start until you resolve the budget incident." })
              : t("common.scopePausedDesc", { defaultValue: "This scope is paused. New heartbeats will not start until you resolve the budget incident." })}
          </div>
        </div>

        <div className="rounded-xl border border-border/60 bg-background/60 p-3">
          <label className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">
            {t("common.newBudgetUsd", { defaultValue: "New budget (USD)" })}
          </label>
          <div className="mt-2 flex flex-col gap-3 sm:flex-row">
            ...
            >
              ...
              {isMutating ? t("common.applying", { defaultValue: "Applying..." }) : t("common.raiseBudgetResume", { defaultValue: "Raise budget & resume" })}
            </Button>
          </div>
          {parsed !== null && parsed <= incident.amountObserved ? (
            <p className="mt-2 text-xs text-red-700 dark:text-red-200/80">
              {t("common.newBudgetMustExceed", { defaultValue: "The new budget must exceed current observed spend." })}
            </p>
          ) : null}
        </div>
        ...
        <div className="flex justify-end">
          <Button variant="ghost" className="text-muted-foreground" disabled={isMutating} onClick={onKeepPaused}>
            {t("common.keepPaused", { defaultValue: "Keep paused" })}
          </Button>
        </div>
      </CardContent>
    </Card>
