...
const MODE_OPTIONS: RadioCardOption[] = [
  {
    value: "open",
    title: t("components.inboxAgentPolicy.anyAgents", { defaultValue: "Any of my agents" }),
    description: t("components.inboxAgentPolicy.anyAgentsDesc", { defaultValue: "Let any agent you manage archive tasks out of your inbox." }),
  },
  {
    value: "allowlist",
    title: t("components.inboxAgentPolicy.chosenAgents", { defaultValue: "Only chosen agents" }),
    description: t("components.inboxAgentPolicy.chosenAgentsDesc", { defaultValue: "Restrict inbox tidying to the agents you pick below." }),
  },
  {
    value: "disabled",
    title: t("components.inboxAgentPolicy.off", { defaultValue: "Off" }),
    description: t("components.inboxAgentPolicy.offDesc", { defaultValue: "Agents can never archive tasks from your inbox." }),
  },
];
...
/**
 * t("components.inboxAgentPolicy.title", { defaultValue: "Let agents tidy my inbox" }) user-settings control. A single
 ...
 * t("components.inboxAgentPolicy.archivedBy", { defaultValue: "Archived by …" }) attribution live elsewhere (inbox rows / properties pane).
 */
export function InboxAgentPolicyControl({ companyId }: { companyId: string | null | undefined }) {
  ...
  if (policyQuery.error) {
    return (
      <div className="text-sm text-destructive">
        {policyQuery.error instanceof Error ? policyQuery.error.message : t("components.inboxAgentPolicy.loadFailed", { defaultValue: "Failed to load inbox agent policy." })}
      </div>
    );
  }

  if (policyQuery.isLoading || !draft) {
    return <div className="text-sm text-muted-foreground">{t("components.inboxAgentPolicy.loading", { defaultValue: "Loading inbox agent policy…" })}</div>;
  }
  ...
  return (
    <section className="space-y-4" aria-label={t("components.inboxAgentPolicy.title", { defaultValue: "Let agents tidy my inbox" })}>
      ...
      <RadioCardGroup
        ariaLabel={t("components.inboxAgentPolicy.policyTitle", { defaultValue: "Inbox agent archiving policy" })}
        value={draft.mode}
      ...
      {updateMutation.error ? (
        <div className="max-w-2xl rounded-md border border-destructive/40 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          {updateMutation.error instanceof Error ? updateMutation.error.message : t("components.inboxAgentPolicy.saveFailed", { defaultValue: "Failed to save inbox agent policy." })}
        </div>
      ) : null}

      <div className="flex max-w-2xl items-center justify-end gap-3">
        {updateMutation.isSuccess && !isDirty ? (
          <span className="text-xs text-muted-foreground" role="status">{t("components.inboxAgentPolicy.saved", { defaultValue: "Saved" })}</span>
        ) : null}
        <Button
          type="button"
        ...
        >
          ...
          {updateMutation.isPending ? t("components.inboxAgentPolicy.saving", { defaultValue: "Saving…" }) : t("components.inboxAgentPolicy.save", { defaultValue: "Save" })}
        </Button>
      </div>
    </section>
  );
