...
function normalizeError(error: unknown): FileViewerErrorShape {
  ...
  return { status: 0, code: "", message: t("components.fileViewer.somethingWrong", { defaultValue: "Something went wrong." }) };
}
...
export function describeDenial(code: string, fallback: string): { title: string; body: string; icon: ReactNode } {
  ...
  if (lower.includes("policy") || lower.includes("denied") || lower.includes("sensitive")) {
    return {
      ...
      title: t("components.fileViewer.blocked", { defaultValue: "Viewer blocked for this file" }),
      body: t("components.fileViewer.blockedDesc", { defaultValue: "This file is not available through the viewer because it may contain sensitive data." }),
    };
  }
  if (lower.includes("outside") || lower.includes("traversal")) {
    return {
      icon: <Ban aria-hidden="true" className="h-6 w-6 text-red-500" />,
      title: t("components.fileViewer.outsideWorkspace", { defaultValue: "Path is outside the workspace" }),
      body: t("components.fileViewer.outsideWorkspaceDesc", { defaultValue: "The viewer can only open files that live under the issue's workspace." }),
    };
  }
  if (lower.includes("archive") || lower.includes("cleaned")) {
    return {
      icon: <FolderOpen aria-hidden="true" className="h-6 w-6 text-muted-foreground" />,
      title: t("components.fileViewer.workspaceGone", { defaultValue: "Workspace is no longer available" }),
      body: t("components.fileViewer.workspaceGoneDesc", { defaultValue: "The isolated worktree for this issue has been cleaned up, so files cannot be previewed." }),
    };
  }
  if (lower.includes("remote")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("components.fileViewer.remoteNotSupported", { defaultValue: "Remote workspace preview not supported" }),
      body: t("components.fileViewer.remoteNotSupportedDesc", { defaultValue: "This workspace is hosted remotely and is not available for inline preview yet." }),
    };
  }
  if (lower.includes("too_large") || lower.includes("size")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("components.fileViewer.tooLarge", { defaultValue: "File is too large to preview" }),
      body: t("components.fileViewer.tooLargeDesc", { defaultValue: "This file exceeds the supported preview size." }),
    };
  }
  if (lower.includes("binary") || lower.includes("unsupported")) {
    return {
      icon: <AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />,
      title: t("components.fileViewer.typeNotSupported", { defaultValue: "Preview not supported for this file type" }),
      body: t("components.fileViewer.typeNotSupportedDesc", { defaultValue: "This file does not have a text, image, or video preview available." }),
    };
  }
  return {
    icon: <Ban aria-hidden="true" className="h-6 w-6 text-red-500" />,
    title: t("components.fileViewer.cantPreview", { defaultValue: "Can't preview this file" }),
    body: fallback || t("components.fileViewer.cantPreviewDesc", { defaultValue: "The viewer was unable to load this file." }),
  };
}
  ...
  return (
    <div className="flex min-h-(--sz-18px) flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-muted-foreground">
      {resolvedResource ? (
        ...
        <span className="h-3 w-28 rounded bg-muted animate-pulse" aria-label={t("components.fileViewer.loadingDetails", { defaultValue: "Loading file details" })} />
      ) : null}
    </div>
  );
}
  ...
  if (resource.previewKind === "image") {
    ...
    if (!dataUrl) {
      return (
        <FileViewerStateView
          icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
          title={t("components.fileViewer.imageUnavailable", { defaultValue: "Image preview unavailable" })}
        />
      );
    }
    return (
  ...
  if (resource.previewKind === "video") {
    ...
    if (!dataUrl) {
      return (
        <FileViewerStateView
          icon={<AlertTriangle aria-hidden="true" className="h-6 w-6 text-amber-500" />}
          title={t("components.fileViewer.videoUnavailable", { defaultValue: "Video preview unavailable" })}
        />
      );
    }
    return (
  ...
  return (
    <div className="relative flex min-h-0 flex-1 flex-col">
      <div className="absolute right-3 top-3 z-20">
        <div
          role="group"
          aria-label={t("components.fileViewer.markdownMode", { defaultValue: "Markdown preview mode" })}
        ...
        >
          <Button
            ...
            size="icon-sm"
            aria-label={t("components.fileViewer.showRendered", { defaultValue: "Show rendered Markdown" })}
            title={t("components.fileViewer.rendered", { defaultValue: "Rendered Markdown" })}
          ...
          <Button
            ...
            size="icon-sm"
            aria-label={t("components.fileViewer.showRaw", { defaultValue: "Show raw Markdown" })}
            title={t("components.fileViewer.raw", { defaultValue: "Raw Markdown" })}
  ...
  if (elapsedMs < 400) {
    return (
      <div className="flex-1 space-y-2 p-6" aria-busy="true" aria-live="polite">
        <span className="sr-only">{t("components.fileViewer.loadingPreview", { defaultValue: "Loading file preview" })}</span>
...
interface FileViewerSheetProps {
  ...
  state?: FileViewerUrlState | null;
  /** When true, renders the t("components.fileViewer.openFile", { defaultValue: "Open file" }) prompt when no file is selected but sheet is open. */
...
export function FileViewerSheet({
  ...
  useEffect(() => {
    if (resolveQuery.isError) {
      ...
      setAnnouncement(normalized.message || t("components.fileViewer.loadFailed", { defaultValue: "Unable to load file." }));
    }
  ...
  useEffect(() => {
    if (contentQuery.isError) {
      ...
      setAnnouncement(normalized.message || t("components.fileViewer.contentFailed", { defaultValue: "Unable to load file content." }));
    }
  ...
  const copyToClipboard = useCallback(async (value: string, field: "content" | "link", message: string) => {
    try {
      ...
      showCopyFeedback(null, t("components.fileViewer.copyFailed", { defaultValue: "Copy failed" }));
    } finally {
  ...
  const handleCopyContent = useCallback(() => {
    ...
    void (async () => {
      ...
      if (!content) {
        showCopyFeedback(null, t("components.fileViewer.contentsUnavailable", { defaultValue: "File contents unavailable" }));
        return;
      }
      const message = content.content.encoding === "base64" ? t("components.fileViewer.copiedData", { defaultValue: "Copied file data" }) : t("components.fileViewer.copiedContents", { defaultValue: "Copied contents" });
  ...
  const handleCopyLink = useCallback(() => {
    ...
    void copyToClipboard(window.location.href, "link", t("components.fileViewer.copiedLink", { defaultValue: "Copied link" }));
  }, [copyToClipboard]);
  ...
  const title = state ? basename(state.path) : t("components.fileViewer.browseWorkspace", { defaultValue: "Browse workspace" });
  const description = state
    ? middleTruncatePath(state.path)
    : t("components.fileViewer.browseHint", { defaultValue: "Search and preview files from this issue's workspace." });
            ...
            <div className="flex shrink-0 items-center gap-1 self-start">
                ...
                downloadUrl ? (
                  ...
                  >
                    <a
                      ...
                      aria-label={t("components.fileViewer.downloadFile", { defaultValue: "Download file" })}
                      title={t("components.fileViewer.downloadFile", { defaultValue: "Download file" })}
                    >
              ...
              {state ? (
                <Button
                  ...
                  aria-label={copiedField === "content" ? t("components.fileViewer.copiedFileContents", { defaultValue: "Copied file contents" }) : t("components.fileViewer.copyFileContents", { defaultValue: "Copy file contents" })}
                  title={copiedField === "content" ? t("components.fileViewer.copiedContents", { defaultValue: "Copied contents" }) : t("components.fileViewer.copyFileContents", { defaultValue: "Copy file contents" })}
                  className="h-7 w-7"
                >
              ...
              {state ? (
                <Button
                  ...
                  aria-label={copiedField === "link" ? t("components.fileViewer.copiedViewLink", { defaultValue: "Copied file view link" }) : t("components.fileViewer.copyViewLink", { defaultValue: "Copy link to this file view" })}
                  title={copiedField === "link" ? t("components.fileViewer.copiedLink", { defaultValue: "Copied link" }) : t("components.fileViewer.copyLink", { defaultValue: "Copy link" })}
                  className="h-7 w-7"
                >
                  {copyingField === "link" ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
              ...
              <Button
                ...
                className="h-7 w-7"
                aria-label={t("components.fileViewer.closeViewer", { defaultValue: "Close file viewer" })}
                title={t("components.fileViewer.close", { defaultValue: "Close" })}
              >
                <X className="h-4 w-4" />
              </Button>
            </div>
          ...
          {state ? (
            <div className="flex min-h-0 flex-1 gap-3 bg-muted/30 p-3">
              ...
              <div
                ...
                aria-label={t("components.fileViewer.resizeTree", { defaultValue: "Resize file tree" })}
  ...
  if (resolveQuery.isError) {
    ...
    if (normalized.status === 404) {
      return (
        <FileViewerStateView
          ...
          title={t("components.fileViewer.fileNotFound", { defaultValue: "File not found" })}
          body={t("components.fileViewer.fileNotFoundDesc", { defaultValue: "That file was not found in the active workspace." })}
          actions={
            <>
              {onFallbackToProject ? (
    ...
    if (normalized.status === 422) {
      return (
        <FileViewerStateView
          icon={<FolderOpen aria-hidden="true" className="h-6 w-6 text-muted-foreground" />}
          title={t("components.fileViewer.noWorkspace", { defaultValue: "No workspace available" })}
          body={t("components.fileViewer.noWorkspaceDesc", { defaultValue: "This issue does not have a workspace that supports preview yet." })}
        />
      );
    }
