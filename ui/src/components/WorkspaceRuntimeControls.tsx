...
function buildJobItem(
  ...
  return {
    ...
    serviceIndex: null,
    disabledReason: command.disabledReason ?? (!command.command ? t("components.workspaceRuntimeControls.jobMissingCommand", { defaultValue: "This job is missing a command." }) : null),
  };
}
...
export function buildWorkspaceRuntimeControlSections(input: {
  ...
  const otherServices = runtimeServices
    ...
    .map((runtimeService) => ({
      ...
      disabledReason: t("components.workspaceRuntimeControls.serviceMismatch", { defaultValue: "This runtime service no longer matches a configured workspace command." }),
    }));
    ...
    <div className={cn("flex gap-2", iconOnly ? "w-auto flex-row flex-wrap" : "w-full flex-col sm:w-auto sm:flex-row sm:flex-wrap")}>
      {actions.map((action) => {
        ...
        const label = action === "run"
          ? t("components.workspaceRuntimeControls.run", { defaultValue: "Run" })
          : action === "start"
            ? t("components.workspaceRuntimeControls.start", { defaultValue: "Start" })
            : action === "stop"
              ? t("components.workspaceRuntimeControls.stop", { defaultValue: "Stop" })
              : t("components.workspaceRuntimeControls.restart", { defaultValue: "Restart" });
            ...
            <div key={item.key} className={cn("border border-border/80 bg-background px-3 py-3", square ? "rounded-none" : "rounded-xl")}>
              <div className="flex flex-col gap-3">
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  ...
                  {item.port ? <div>{t("pages.companyEnvironments.port")}{item.port}</div> : null}
...
export function WorkspaceRuntimeControls({
  ...
  isPending = false,
  pendingRequest = null,
  serviceEmptyMessage = t("components.workspaceRuntimeControls.noServices", { defaultValue: "No services are configured for this workspace." }),
  jobEmptyMessage = t("components.workspaceRuntimeControls.noJobs", { defaultValue: "No one-shot jobs are configured for this workspace." }),
  emptyMessage,
  disabledHint = null,
  onAction,
  className,
  ...
  return (
    <div className={cn("space-y-4", className)}>
        ...
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {runningCount > 0 ? `${runningCount} services running` : t("components.workspaceRuntimeControls.noServicesRunning", { defaultValue: "No services running" })}
            </Badge>
            <span className="text-xs text-muted-foreground">
              {resolvedSections.jobs.length > 0
                ...
                : t("components.workspaceRuntimeControls.independentHint", { defaultValue: "Each command can be controlled independently." })}
            </span>
          </div>
      ...
      <CommandSection
        ...
        description={t("components.workspaceRuntimeControls.servicesDesc", { defaultValue: "Long-running commands that Paperclip can supervise for this workspace." })}
      ...
      <CommandSection
        ...
        description={t("components.workspaceRuntimeControls.jobsDesc", { defaultValue: "One-shot commands that run now and exit when they finish." })}
      ...
      {resolvedSections.otherServices.length > 0 ? (
        <CommandSection
          ...
          description={t("components.workspaceRuntimeControls.untrackedDesc", { defaultValue: "Running services that no longer match the current workspace command config." })}
