  ...
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">{t("ui.components.financekindcard.financial-event-mix")}</CardTitle>
        <CardDescription>{t("ui.components.financekindcard.account-level-charges-grouped")}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-2 px-4 pb-4 pt-3">
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t("ui.components.financekindcard.no-finance-events-period")}</p>
        ) : (
          rows.map((row) => (
            <div
              key={row.eventKind}
