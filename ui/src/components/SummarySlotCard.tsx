...
export function SummarySlotCard({
  ...
  const generateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("components.summarySlotCard.startFailed", { defaultValue: "Summary generation could not be started." }));
    },
  });
  ...
  const latestSelectLabel = latestDocument ? latestRevisionOptionLabel(latestDocument, latestRevision) : t("components.summarySlotCard.latest", { defaultValue: "Latest" });
  ...
  return (
    <section className={cn("space-y-4", className)}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {isGenerating ? <Badge variant="secondary">{t("components.summarySlotCard.generating2", { defaultValue: "Generating" })}</Badge> : null}
            {displayingHistoricalRevision ? <Badge variant="outline">{t("components.summarySlotCard.historicalRevision", { defaultValue: "Historical revision" })}</Badge> : null}
            {latestDocument && !displayingHistoricalRevision ? <Badge variant="outline">{t("components.summarySlotCard.latestRevision", { defaultValue: "Latest revision" })}</Badge> : null}
          </div>
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          {displayingHistoricalRevision ? (
            ...
            >
              {t("components.summarySlotCard.latest")}</Button>
          ) : null}
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1 text-sm">
                <p className="font-medium text-foreground">
                  {needsSetup.status === "pending_approval"
                    ? t("components.summarySlotCard.setupPending", { defaultValue: "Summarizer setup is pending approval" })
                    : t("components.summarySlotCard.setUpSummarizer", { defaultValue: "Set up the Summarizer" })}
                </p>
                <p className="text-muted-foreground">
                  {t("ui.components.summaryslotcard.summaries-generated-paperclip-built")}</p>
              </div>
            </div>
            {needsSetup.status === "pending_approval" ? null : (
              <Button type="button" size="sm" onClick={() => setConfigureOpen(true)}>
                {t("ui.components.summaryslotcard.set-up-summarizer")}</Button>
            )}
          </div>
          {companyId ? (
      ...
      {!needsSetup && summarizerState?.status === "paused" && summarizerState.agent ? (
        <InlineBanner
          tone="warning"
          title={t("components.summarySlotCard.summarizerPaused", { defaultValue: "Summarizer is paused" })}
          actions={
            <Button
              type="button"
              size="sm"
            ...
            >
              {resumeSummarizer.isPending ? t("components.summarySlotCard.resuming", { defaultValue: "Resuming..." }) : t("components.summarySlotCard.resumeAgent", { defaultValue: "Resume agent" })}
            </Button>
          }
        >
          {t("ui.components.summaryslotcard.existing-summaries-remain-readable")}</InlineBanner>
      ) : null}

      {actionError ? (
        <InlineBanner tone="warning" title={t("components.summarySlotCard.requestFailed", { defaultValue: "Summary request failed" })}>
          {actionError}
        </InlineBanner>
      ) : null}
      ...
      {slotQuery.isError ? (
        <InlineBanner
          tone="warning"
          title={t("components.summarySlotCard.loadFailed", { defaultValue: "Summary could not be loaded" })}
          actions={
            <Button type="button" size="sm" variant="outline" onClick={() => void slotQuery.refetch()}>
              {t("components.issueProperties.retry")}</Button>
          }
        >
          {slotQuery.error instanceof Error ? slotQuery.error.message : t("components.summarySlotCard.retryLoad", { defaultValue: "Try loading the summary again." })}
        </InlineBanner>
      ) : null}
      ...
      {!slotQuery.isError && generationFailed ? (
        <InlineBanner
          tone="danger"
          title={t("components.summarySlotCard.generationFailed", { defaultValue: "Summary generation failed" })}
          actions={
            <Button
              type="button"
              size="sm"
            ...
            >
              {generateMutation.isPending ? t("components.summarySlotCard.retrying", { defaultValue: "Retrying..." }) : t("components.summarySlotCard.retry", { defaultValue: "Retry" })}
            </Button>
          }
        >
          {slotQuery.data?.slot?.failureReason ?? t("components.summarySlotCard.taskEnded", { defaultValue: "The generation task ended before writing a summary." })}
        </InlineBanner>
      ) : null}
      ...
      {!slotQuery.isError && isGenerating && generatingIssue ? (
        <div className="flex items-start gap-3 text-sm">
          ...
          <div className="min-w-0 space-y-1">
            <p className="font-medium text-foreground">{t("components.summarySlotCard.generatingSummary", { defaultValue: "Generating summary" })}</p>
            ...
            <p className="text-muted-foreground">
              {t("ui.components.summaryslotcard.summarizer-working")}{" "}
      ...
      {!slotQuery.isError && !latestDocument && !isGenerating && !generationFailed && canGenerateFirstSummary ? (
        <div className="flex flex-col items-start gap-3 rounded-lg border border-border bg-muted/30 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1 text-sm">
            <p className="font-medium text-foreground">{t("components.summarySlotCard.noSummary", { defaultValue: "No summary yet" })}</p>
            <p className="text-muted-foreground">{t("components.summarySlotCard.generateHint", { defaultValue: "Generate a concise status snapshot for this surface." })}</p>
          </div>
          <Button
            type="button"
            size="sm"
          ...
          >
            ...
            {generateMutation.isPending ? t("components.summarySlotCard.generating", { defaultValue: "Generating..." }) : t("components.summarySlotCard.generateSummary", { defaultValue: "Generate summary" })}
          </Button>
        </div>
      ) : null}
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-3 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
            <span title={formatDateTime(historicalRevision?.createdAt ?? latestRevision?.createdAt ?? latestDocument.updatedAt)}>
              {t("components.issueProperties.updated")}{relativeTime(historicalRevision?.createdAt ?? latestRevision?.createdAt ?? latestDocument.updatedAt)}
            </span>
            ...
            {revisions.length > 1 ? (
              ...
              >
                <SelectTrigger
                  ...
                  aria-label={t("components.summarySlotCard.selectRevision", { defaultValue: "Select summary revision" })}
