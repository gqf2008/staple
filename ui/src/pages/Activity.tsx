...
export function Activity() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("pages.activity.title", { defaultValue: "Activity" }) }]);
  }, [setBreadcrumbs]);
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={History} message={t("pages.activity.selectCompany", { defaultValue: "Select a company to view activity." })} />;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-end">
        <Select value={filter} onValueChange={setFilter}>
          <SelectTrigger className="w-(--sz-140px) h-8 text-xs">
            <SelectValue placeholder={t("pages.activity.filterByType", { defaultValue: "Filter by type" })} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="all">{t("pages.activity.allTypes", { defaultValue: "All types" })}</SelectItem>
      ...
      {filtered && filtered.length === 0 && (
        <EmptyState icon={History} message={t("pages.activity.none", { defaultValue: "No activity yet." })} />
      )}
