...
function mapInviteAuthFeedback(
  ...
  return {
    tone: "error",
    message: message ?? t("pages.inviteLanding.authFailed", { defaultValue: "Authentication failed" }),
  };
}
...
function AwaitingJoinApprovalPanel({
  ...
  const approverLabel = invitedByUserName ?? t("ui.pages.invitelanding.fallback-company-admin");
...
export function InviteLandingPage() {
  ...
  const companyDisplayName = companyName || t("ui.pages.invitelanding.fallback-paperclip-company");
  ...
  const sessionLabel =
    ...
    t("ui.pages.invitelanding.fallback-account");
  ...
  const acceptMutation = useMutation({
    mutationFn: async () => {
      if (!invite) throw new Error(t("pages.inviteLanding.inviteNotFound", { defaultValue: "Invite not found" }));
      if (isCheckingExistingMembership) {
        throw new Error(t("pages.inviteLanding.checkingAccess", { defaultValue: "Checking your company access. Try again in a moment." }));
      }
      if (isCurrentMember) {
        throw new Error(t("pages.inviteLanding.alreadyMember", { defaultValue: "This account already belongs to the company." }));
      }
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : t("pages.inviteLanding.acceptFailed", { defaultValue: "Failed to accept invite" }));
    },
  });
  ...
  const joinButtonLabel = useMemo(() => {
    ...
    if (invite.inviteType === "bootstrap_ceo") return t("pages.inviteLanding.acceptInvite", { defaultValue: "Accept invite" });
    if (showsAgentForm) return t("pages.inviteLanding.submitRequest", { defaultValue: "Submit request" });
    return sessionQuery.data ? t("pages.inviteLanding.acceptInvite", { defaultValue: "Accept invite" }) : t("pages.inviteLanding.continue", { defaultValue: "Continue" });
        ...
        <div className="border border-border bg-card p-6" data-testid="invite-error">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {inviteJoinRequestStatus === "rejected"
              ? t("pages.inviteLanding.joinNotApproved", { defaultValue: "This join request was not approved." })
              : t("pages.inviteLanding.inviteUsed", { defaultValue: "This invite has already been used." })}
          </p>
        </div>
      </div>
    );
        ...
        <div className="grid gap-6 lg:grid-cols-(--gtc-36)">
          <section className={`${panelClassName} space-y-6`}>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="border border-zinc-800 p-3">
                <div className="text-xs uppercase tracking-(--tracking-caps) text-zinc-500">{t("pages.inviteLanding.company", { defaultValue: "Company" })}</div>
              ...
              <div className="border border-zinc-800 p-3">
                ...
                <div className="mt-1 text-sm text-zinc-100">
                  {showsAgentForm ? t("pages.inviteLanding.agentJoinRequest", { defaultValue: "Agent join request" }) : requestedHumanRole ?? t("pages.inviteLanding.companyAccess", { defaultValue: "Company access" })}
                </div>
              </div>
              <div className="border border-zinc-800 p-3">
            ...
            {inviteMessage ? (
              <div className="border border-amber-500/40 bg-amber-500/10 p-4">
                <div className="text-xs uppercase tracking-(--tracking-caps) text-amber-200/80">{t("pages.inviteLanding.inviterMessage", { defaultValue: "Message from inviter" })}</div>
          ...
          <section className={`${panelClassName} h-fit`}>
            {showsAgentForm ? (
              <div className="space-y-4">
                ...
                >
                  {acceptMutation.isPending ? t("pages.inviteLanding.working", { defaultValue: "Working..." }) : joinButtonLabel}
                </Button>
              </div>
            ) : requiresHumanAccount ? (
              <div className="space-y-5">
                <div>
                  <h2 className="text-lg font-semibold">
                    {authMode === "sign_up" ? t("pages.inviteLanding.createAccount", { defaultValue: "Create your account" }) : t("pages.inviteLanding.signInToContinue", { defaultValue: "Sign in to continue" })}
                  </h2>
                  <p className="mt-1 text-sm text-zinc-400">
                    {authMode === "sign_up"
                ...
                <form
                  ...
                  onSubmit={(event) => {
                    ...
                    if (!authCanSubmit) {
                      setAuthFeedback({ tone: "error", message: t("pages.inviteLanding.requiredFields", { defaultValue: "Please fill in all required fields." }) });
                      return;
                    }
                    authMutation.mutate();
                  }}
                ...
                >
                  ...
                  >
                    {authMutation.isPending
                      ...
                      : authMode === "sign_in"
                        ? t("pages.inviteLanding.signInAndContinue", { defaultValue: "Sign in and continue" })
                        : t("pages.inviteLanding.createAndContinue", { defaultValue: "Create account and continue" })}
                  </Button>
                </form>
              ...
              <div className="space-y-4">
                <div>
                  <h2 className="text-lg font-semibold">
                    {isCurrentMember
                      ? t("pages.inviteLanding.alreadyInCompany", { defaultValue: "Already in this company" })
                      : shouldAutoAcceptHumanInvite
                      ? t("pages.inviteLanding.completingAccess", { defaultValue: "Completing company access" })
                      : invite.inviteType === "bootstrap_ceo"
                        ? t("pages.inviteLanding.acceptBootstrap", { defaultValue: "Accept bootstrap invite" })
                ...
                {shouldAutoAcceptHumanInvite ? (
                  <div className="text-sm text-zinc-400">
                    {acceptMutation.isPending ? t("pages.inviteLanding.submitting", { defaultValue: "Submitting request..." }) : t("pages.inviteLanding.finishingSignIn", { defaultValue: "Finishing sign-in..." })}
                  </div>
                ) : (
                  <Button
                    className="w-full rounded-none"
