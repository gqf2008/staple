...
export function AddConnectionDialog({
  ...
  const [pendingHeader, setPendingHeader] = useState(t("pages.tools.connectionDialogs.authorization", { defaultValue: "Authorization" }));
  ...
  const create = useMutation({
    ...
    onError: (err) =>
      pushToast({
        title: t("pages.tools.connectionDialogs.createFailed", { defaultValue: "Could not create connection" }),
  ...
  const probe = useMutation({
    ...
    onError: (err) =>
      pushToast({
        title: t("pages.tools.connectionDialogs.probeFailed", { defaultValue: "Probe failed" }),
        body: err instanceof ApiError ? err.message : String(err),
        tone: "error",
      }),
  });
  ...
  const activate = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: t("pages.tools.connectionDialogs.activated", { defaultValue: "Connection activated" }), tone: "success" });
      onClose();
    },
    onError: (err) =>
      pushToast({
        title: t("pages.tools.connectionDialogs.activationFailed", { defaultValue: "Activation failed" }),
        body: err instanceof ApiError ? err.message : String(err),
        tone: "error",
      }),
  });
  ...
  const locked = !!draft;
  const inferredType = transport === "mcp_remote" ? t("pages.tools.connectionDialogs.mcpHttp", { defaultValue: "MCP HTTP" }) : t("pages.tools.connectionDialogs.mcpStdio", { defaultValue: "MCP stdio" });
        ...
        <div className="space-y-3">
          ...
          {step === 1 && !locked ? (
            <>
              <div className="space-y-1.5">
                <Label>{t("pages.tools.connectionDialogs.application", { defaultValue: "Application" })}</Label>
                <Select value={applicationMode} onValueChange={(v) => setApplicationMode(v as "existing" | "new")}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="existing">{t("pages.tools.connectionDialogs.useExisting", { defaultValue: "Use existing application" })}</SelectItem>
                    <SelectItem value="new">{t("pages.tools.connectionDialogs.createNewApp", { defaultValue: "Create new application" })}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {applicationMode === "existing" ? (
                <div className="space-y-1.5">
                  <Label>{t("pages.tools.connectionDialogs.existingApp", { defaultValue: "Existing application" })}</Label>
                  <Select value={applicationId} onValueChange={setApplicationId}>
                    <SelectTrigger>
                      <SelectValue placeholder={t("pages.tools.connectionDialogs.selectApplication", { defaultValue: "Select an application" })} />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="app-name">{t("pages.tools.connectionDialogs.newAppName", { defaultValue: "New application name" })}</Label>
                  <Input
                    id="app-name"
          ...
          {step === 2 || locked ? (
            <>
              ...
              <div className="space-y-1.5">
                <Label htmlFor="conn-name">{t("pages.tools.connectionDialogs.connectionName", { defaultValue: "Connection name" })}</Label>
                <Input
                  id="conn-name"
                  value={name}
              ...
              <div className="space-y-1.5">
                <Label>{t("pages.tools.connectionDialogs.transport", { defaultValue: "Transport" })}</Label>
                <Select
                  value={transport}
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="mcp_remote">{t("pages.tools.connectionDialogs.remoteHttp", { defaultValue: "Remote HTTP (no local process)" })}</SelectItem>
                    <SelectItem value="local_stdio">{t("pages.tools.connectionDialogs.localStdio", { defaultValue: "Local stdio (approved template)" })}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {transport === "mcp_remote" ? (
                <div className="space-y-1.5">
                  <Label htmlFor="conn-url">{t("pages.tools.connectionDialogs.endpointUrl", { defaultValue: "Endpoint URL" })}</Label>
                  <Input
                    id="conn-url"
                    value={endpointUrl}
                ...
                <div className="space-y-1.5">
                  <Label>{t("pages.tools.connectionDialogs.commandTemplate", { defaultValue: "Command template" })}</Label>
                  <Select value={templateId} onValueChange={setTemplateId} disabled={locked}>
                    <SelectTrigger>
                      <SelectValue placeholder={t("pages.tools.connectionDialogs.selectTemplate", { defaultValue: "Select an approved template" })} />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div className="space-y-1.5">
                <Label>{t("pages.tools.connectionDialogs.credentialRefs", { defaultValue: "Credential references" })}</Label>
                {creds.length > 0 ? (
                    ...
                    <div className="flex items-end gap-2">
                      <div className="flex-1 space-y-1">
                        <Select value={pendingSecretId} onValueChange={setPendingSecretId}>
                          <SelectTrigger>
                            <SelectValue placeholder={t("pages.tools.connectionDialogs.selectSecret", { defaultValue: "Select a vault secret" })} />
                          </SelectTrigger>
                          <SelectContent>
                            {(secrets.data ?? []).map((s) => (
                      ...
                      <Input
                        ...
                        placeholder={t("pages.tools.connectionDialogs.header", { defaultValue: "Header" })}
                        className="w-32"
          ...
          {locked ? (
            probe.isPending ? (
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <LoadingState label={t("pages.tools.connectionDialogs.probing", { defaultValue: "Probing connection…" })} />
              </div>
            ) : probe.isError ? (
              ...
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <div className="flex items-center gap-2 text-sm">
                  <span className="font-medium text-foreground">{t("pages.tools.connectionDialogs.probeResult", { defaultValue: "Probe result" })}</span>
          ...
          {step === 1 && !locked ? (
            ...
            <>
              <Button variant="outline" disabled={probe.isPending} onClick={() => draft && probe.mutate(draft.id)}>
                ...
                {probe.isPending ? t("pages.tools.connectionDialogs.probing2", { defaultValue: "Probing…" }) : t("pages.tools.connectionDialogs.reprobe", { defaultValue: "Re-probe" })}
              </Button>
              <Button disabled={activate.isPending || probe.isPending} onClick={() => draft && activate.mutate(draft.id)}>
                {activate.isPending ? t("pages.tools.connectionDialogs.activating", { defaultValue: "Activating…" }) : t("pages.tools.connectionDialogs.activate", { defaultValue: "Activate" })}
              </Button>
            </>
          )}
        </DialogFooter>
