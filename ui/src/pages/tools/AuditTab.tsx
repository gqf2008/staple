...
const OUTCOME_META: Record<ToolAuditOutcome, { label: string; status: string }> = {
  ...
  unknown: { label: t("pages.tools.audit.recorded", { defaultValue: "Recorded" }), status: "unchecked" },
};
...
const WINDOW_FILTERS: { value: ToolAuditWindow; label: string }[] = [
  { value: "1h", label: t("pages.tools.audit.last1h", { defaultValue: "Last 1 hour" }) },
  { value: "24h", label: t("pages.tools.audit.last24h", { defaultValue: "Last 24 hours" }) },
  { value: "7d", label: t("pages.tools.audit.last7d", { defaultValue: "Last 7 days" }) },
  { value: "30d", label: t("pages.tools.audit.last30d", { defaultValue: "Last 30 days" }) },
];
...
function plainReason(event: ToolGatewayActivityEvent): string {
  ...
  if (code === "permitted_connections_not_installed") {
    return t("pages.tools.audit.notInstalledDesc", { defaultValue: "Permitted connections were not installed, so their tools were not added to this run." });
  }
  switch (event.normalizedOutcome) {
    case "allowed":
      return t("pages.tools.audit.allowedDesc", { defaultValue: "Allowed by your rules." });
    case "blocked":
    ...
    case "asked_first":
      return t("pages.tools.audit.askedFirstDesc", { defaultValue: "Held for someone to approve before it could run." });
    case "waiting":
      return t("pages.tools.audit.waitingDesc", { defaultValue: "Waiting — the app it needs wasn't ready yet." });
    case "failed":
      return t("pages.tools.audit.failedDesc", { defaultValue: "The app was allowed to run it, but returned an error." });
    default:
      return t("pages.tools.audit.recordedDesc", { defaultValue: "Recorded by Paperclip." });
  }
}
          ...
          <div>
            ...
            {detailsOpen ? (
              <div className="mt-2 space-y-1.5 text-xs">
                ...
                <DetailFact label={t("pages.tools.audit.reasonCode", { defaultValue: "Reason code" })} value={reasonCode} mono />
                ...
                {isRuntimeMcpDeliveryDiagnostic ? (
                  <>
                    <DetailFact label={t("pages.tools.audit.deliveredMcp", { defaultValue: "Delivered MCP servers" })} value="0" mono />
                    {permittedNotInstalledConnections.map((connection) => {
                      const connectionId = detailString(connection, "id");
                      const connectionName = detailString(connection, "name") ?? t("pages.tools.audit.unnamedConnection", { defaultValue: "Unnamed connection" });
                      return connectionId ? (
                        <div key={connectionId} className="flex gap-2">
                          <span className="shrink-0 text-muted-foreground">{t("pages.tools.audit.notInstalled", { defaultValue: "Not installed" })}</span>
                ...
                {argumentsText ? (
                  <div className="space-y-1">
                    <span className="text-muted-foreground">{t("pages.tools.audit.parametersRedacted", { defaultValue: "Parameters (redacted)" })}</span>
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Select value={app} onValueChange={setApp}>
          <SelectTrigger className="w-40">
            <SelectValue placeholder={t("pages.tools.audit.app", { defaultValue: "App" })} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value={ALL}>{t("pages.tools.audit.allApps", { defaultValue: "All apps" })}</SelectItem>
        ...
        <Select value={agent} onValueChange={setAgent}>
          <SelectTrigger className="w-40">
            <SelectValue placeholder={t("pages.tools.audit.agent", { defaultValue: "Agent" })} />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value={ALL}>{t("pages.tools.audit.allAgents", { defaultValue: "All agents" })}</SelectItem>
            {(agents.data ?? []).map((a) => (
              <SelectItem key={a.id} value={a.id}>
                {a.name}
              </SelectItem>
        ...
        <Input
          placeholder={t("pages.tools.audit.searchPlaceholder", { defaultValue: "Search activity…" })}
          value={searchInput}
      ...
      {activity.hasNextPage ? (
        <div className="flex justify-center">
          ...
          >
            {activity.isFetchingNextPage ? t("pages.tools.audit.loading", { defaultValue: "Loading…" }) : t("pages.tools.audit.loadMore", { defaultValue: "Load more" })}
          </Button>
        </div>
      ) : null}
