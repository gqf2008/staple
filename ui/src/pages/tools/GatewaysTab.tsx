...
function formatOwner(gateway: ToolMcpGatewayWithTokens, agentNames: Map<string, string>) {
  ...
  return t("pages.tools.gateways.board", { defaultValue: "Board" });
}
...
function formatScope(
  ...
  return t("pages.tools.gateways.company", { defaultValue: "Company" });
}

function formatAllowedTools(profile: ToolProfileWithDetails | undefined) {
  if (!profile) return t("pages.tools.gateways.profileUnavailable", { defaultValue: "Profile unavailable" });
  ...
  return allowed === 0 ? t("pages.tools.gateways.noToolsAllowed", { defaultValue: "No tools allowed" }) : `${pluralize(allowed, "tool")} allowed`;
}
...
export function GatewaysTab({ companyId }: { companyId: string }) {
  ...
  const createGatewayMutation = useMutation({
    ...
    onSuccess: async (gateway) => {
      ...
      setCreating(false);
      pushToast({ title: t("pages.tools.gateways.gatewayCreated", { defaultValue: "Gateway created" }), body: gateway.name, tone: "success" });
      await invalidateGateways();
    },
    onError: (error) => {
      pushToast({ title: t("pages.tools.gateways.gatewayCreateFailed", { defaultValue: "Gateway was not created" }), body: error instanceof Error ? error.message : String(error), tone: "error" });
    },
  });
  ...
  const createTokenMutation = useMutation({
    ...
    onSuccess: async (token) => {
      ...
      pushToast({ title: t("pages.tools.gateways.tokenIssued", { defaultValue: "Token issued" }), body: `${token.name} was created. Copy it now; it will not be shown again.`, tone: "success" });
      await invalidateGateways();
    },
    onError: (error) => {
      pushToast({ title: t("pages.tools.gateways.tokenIssueFailed", { defaultValue: "Token was not issued" }), body: error instanceof Error ? error.message : String(error), tone: "error" });
    },
  });
  ...
  const revokeTokenMutation = useMutation({
    ...
    onSuccess: async (token) => {
      ...
      pushToast({ title: t("pages.tools.gateways.tokenRevoked", { defaultValue: "Token revoked" }), body: token.name, tone: "success" });
      await invalidateGateways();
    },
    onError: (error) => {
      pushToast({ title: t("pages.tools.gateways.tokenRevokeFailed", { defaultValue: "Token was not revoked" }), body: error instanceof Error ? error.message : String(error), tone: "error" });
    },
  });
  ...
  async function copyText(value: string, label: string) {
    try {
      if (typeof navigator === "undefined" || !navigator.clipboard?.writeText) {
        throw new Error(t("pages.tools.gateways.clipboardUnavailable", { defaultValue: "Clipboard access is unavailable." }));
      }
      await navigator.clipboard.writeText(value);
      pushToast({ title: t("pages.tools.gateways.copied", { defaultValue: "Copied to clipboard" }), body: label, tone: "success" });
    } catch (error) {
      pushToast({ title: t("pages.tools.gateways.copyFailed", { defaultValue: "Copy failed" }), body: error instanceof Error ? error.message : t("pages.tools.gateways.clipboardUnavailable", { defaultValue: "Clipboard access is unavailable." }), tone: "error" });
    }
  }
  ...
  function submitCreateToken(event: FormEvent<HTMLFormElement>, gatewayId: string) {
    ...
    if (draft.allowedActions.length === 0) {
      pushToast({ title: t("pages.tools.gateways.pickTokenActions", { defaultValue: "Pick token actions" }), body: t("pages.tools.gateways.actionsRequired", { defaultValue: "Gateway tokens need at least one allowed MCP action." }), tone: "warn" });
      return;
    }
    createTokenMutation.mutate(gatewayId);
  }

  if (gatewaysQuery.isLoading) return <LoadingState label={t("pages.tools.gateways.loadingGateways", { defaultValue: "Loading gateways..." })} />;
        ...
        <form className="space-y-3 rounded-md border border-border p-4" onSubmit={submitCreateGateway}>
          <div className="grid gap-3 md:grid-cols-(--gtc-60)">
            <label className="space-y-1.5 text-sm">
              <span className="text-xs font-medium text-muted-foreground">{t("pages.tools.gateways.gatewayName", { defaultValue: "Gateway name" })}</span>
              <input
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                value={createDraft.name}
                onChange={(event) => setCreateDraft((current) => ({ ...current, name: event.target.value }))}
                placeholder={t("pages.tools.gateways.namePlaceholder", { defaultValue: "Engineering laptops" })}
                required
              />
            </label>
            <label className="space-y-1.5 text-sm">
              <span className="text-xs font-medium text-muted-foreground">{t("pages.tools.gateways.accessProfile", { defaultValue: "Access profile" })}</span>
              <select
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              ...
              >
                <option value="" disabled>
                  {profileLoading ? t("pages.tools.gateways.loadingProfiles", { defaultValue: "Loading profiles..." }) : t("pages.tools.gateways.chooseProfile", { defaultValue: "Choose a profile" })}
                </option>
          ...
          <label className="space-y-1.5 text-sm">
            <span className="text-xs font-medium text-muted-foreground">{t("pages.tools.gateways.description", { defaultValue: "Description" })}</span>
            <textarea
              className="min-h-20 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              value={createDraft.description}
              onChange={(event) => setCreateDraft((current) => ({ ...current, description: event.target.value }))}
              placeholder={t("pages.tools.gateways.descHint", { defaultValue: "Who this endpoint is for and when it should be rotated." })}
            />
          </label>
          {activeProfiles.length === 0 && !profileLoading ? (
            <p className="text-xs text-muted-foreground">{t("pages.tools.gateways.profileRequired", { defaultValue: "Create an access profile before adding a gateway." })}</p>
          ) : null}
            ...
            return (
              <section key={gateway.id} className="space-y-4 p-4">
                ...
                <dl className="grid gap-x-4 gap-y-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("pages.tools.gateways.owner", { defaultValue: "Owner" })}</dt>
                    <dd className="mt-0.5 text-foreground">{formatOwner(gateway, agentNames)}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("pages.tools.gateways.scope", { defaultValue: "Scope" })}</dt>
                    <dd className="mt-0.5 text-foreground">{formatScope(gateway, projectNames, agentNames)}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("pages.tools.gateways.allowedTools", { defaultValue: "Allowed tools" })}</dt>
                    <dd className="mt-0.5 text-foreground">
                      {profile ? `${formatAllowedTools(profile)} via ${profile.name}` : `Profile ${shortId(gateway.profileId)}`}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium text-muted-foreground">{t("pages.tools.gateways.lastActivity", { defaultValue: "Last activity" })}</dt>
                    <dd className="mt-0.5 text-foreground">
                      {lastActivity ? <RelativeTime value={lastActivity} /> : t("pages.tools.gateways.neverUsed", { defaultValue: "Never used" })}
                    </dd>
                  </div>
                </dl>
                ...
                {issuingGatewayId === gateway.id ? (
                  <form className="space-y-3 rounded-md border border-border p-3" onSubmit={(event) => submitCreateToken(event, gateway.id)}>
                    <div className="grid gap-3 md:grid-cols-2">
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("pages.tools.gateways.tokenName", { defaultValue: "Token name" })}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          ...
                          placeholder={t("pages.tools.gateways.tokenNamePlaceholder", { defaultValue: "Dotta's MacBook" })}
                          required
                        />
                      </label>
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("pages.tools.gateways.clientLabel", { defaultValue: "Client label" })}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          ...
                          placeholder={t("pages.tools.gateways.clientLabelPlaceholder", { defaultValue: "Cursor on work laptop" })}
                          required
                        />
                      </label>
                    </div>
                    <div className="grid gap-3 md:grid-cols-[1fr_auto]">
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("pages.tools.gateways.ownerNote", { defaultValue: "Owner note" })}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          ...
                          placeholder={t("pages.tools.gateways.ownerNoteHint", { defaultValue: "Who owns this token and why it exists" })}
                          required
                        />
                      </label>
                      <label className="space-y-1.5 text-sm">
                        <span className="text-xs font-medium text-muted-foreground">{t("pages.tools.gateways.expires", { defaultValue: "Expires" })}</span>
                        <input
                          className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                          type="date"
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                      ...
                      >
                        {createTokenMutation.isPending ? t("pages.tools.gateways.issuing", { defaultValue: "Issuing..." }) : t("pages.tools.gateways.issueToken", { defaultValue: "Issue token" })}
                      </Button>
                    </div>
                  </form>
                ) : null}
                ...
                <div className="grid gap-3 md:grid-cols-2">
                            ...
                            <div key={token.id} className="space-y-1 py-1">
                              ...
                              {confirming ? (
                                <div className="flex flex-wrap items-center justify-end gap-2 text-xs text-muted-foreground">
                                  <span>{t("pages.tools.gateways.revokeConfirm", { defaultValue: "Revoke this token now?" })}</span>
                                  <Button
                                    type="button"
                                    variant="ghost"
                                    size="sm"
                  ...
                  <div>
                    <div className="mb-1.5 text-xs font-medium text-muted-foreground">{t("pages.tools.gateways.clientSnippets", { defaultValue: "Client snippets" })}</div>
                    <div className="space-y-1 text-sm">
                      {snippets.length === 0 ? (
                        <p className="text-muted-foreground">{t("pages.tools.gateways.noSnippets", { defaultValue: "No snippets available." })}</p>
                      ) : (
