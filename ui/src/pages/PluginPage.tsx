  ...
  if (!resolvedCompanyId) {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">{t("ui.pages.companysettingspluginpage.select-company-view-page")}</p>
      </div>
    );
  }

  if (!contributions) {
    return <div className="text-sm text-muted-foreground">{t("components.secretBindingPicker.loading")}</div>;
  }
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          {t("ui.pages.pluginpage.multiple-plugins-declare-route")}<code>{pluginRoutePath}</code>{t("ui.pages.pluginpage.use-plugin-id-route")}</div>
      );
    }
  }
