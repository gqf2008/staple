...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  ...
  switch (sourceBadge) {
    ...
    case "local":
      return { icon: Folder, label: sourceLabel ?? t("pages.companySkills.folder", { defaultValue: "Folder" }), managedLabel: t("pages.companySkills.folderManaged", { defaultValue: "Folder managed" }) };
    case "paperclip":
      return { icon: Paperclip, label: sourceLabel ?? t("pages.companySkills.paperclip", { defaultValue: "Paperclip" }), managedLabel: t("pages.companySkills.paperclipManaged", { defaultValue: "Paperclip managed" }) };
    default:
      return { icon: Boxes, label: sourceLabel ?? t("pages.companySkills.catalog", { defaultValue: "Catalog" }), managedLabel: t("pages.companySkills.catalogManaged", { defaultValue: "Catalog managed" }) };
  }
}
...
const SOURCE_FILTER_LABELS: Record<SourceFilter, string> = {
  all: t("pages.companySkills.all", { defaultValue: "All" }),
  company: t("pages.companySkills.company", { defaultValue: "Company" }),
  bundled: t("pages.companySkills.bundled", { defaultValue: "Bundled" }),
  optional: t("pages.companySkills.optional", { defaultValue: "Optional" }),
  external: t("pages.companySkills.external", { defaultValue: "External" }),
};
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          title={activeFilterCount > 0 ? `Filters: ${activeFilterCount}` : t("pages.companySkills.filter", { defaultValue: "Filter" })}
        >
      ...
      <DropdownMenuContent align="end" className="w-48">
        <DropdownMenuLabel>{t("pages.companySkills2.source", { defaultValue: "Source" })}</DropdownMenuLabel>
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="max-h-(--sz-calc-32) w-56 overflow-y-auto">
        <DropdownMenuLabel>{t("pages.companySkills2.type", { defaultValue: "Type" })}</DropdownMenuLabel>
        <DropdownMenuRadioGroup value={kindFilter} onValueChange={(next) => onKindChange(next as "all" | "bundled" | "optional")}>
          <DropdownMenuRadioItem value="all">{t("pages.companySkills.all", { defaultValue: "All" })}</DropdownMenuRadioItem>
          <DropdownMenuRadioItem value="bundled">{t("pages.companySkills.bundled", { defaultValue: "Bundled" })}</DropdownMenuRadioItem>
          <DropdownMenuRadioItem value="optional">{t("pages.companySkills.optional", { defaultValue: "Optional" })}</DropdownMenuRadioItem>
        </DropdownMenuRadioGroup>
        <DropdownMenuSeparator />
        <DropdownMenuLabel>{t("pages.companySkills2.category", { defaultValue: "Category" })}</DropdownMenuLabel>
...
function TrustChip({ level }: { level: CompanySkillTrustLevel }) {
  const map = {
    markdown_only: {
      icon: ShieldCheck,
      label: t("pages.companySkills.markdownOnly", { defaultValue: "Markdown only" }),
      tooltip: t("pages.companySkills.textOnlyDesc", { defaultValue: "Text only — no scripts, no binaries, no assets." }),
      className: "border-border bg-muted/40 text-muted-foreground",
    },
    assets: {
      icon: Folder,
      label: t("pages.companySkills.includesAssets", { defaultValue: "Includes assets" }),
      tooltip: t("pages.companySkills.includesAssetsDesc", { defaultValue: "Ships images, fonts, or other non-script files." }),
      className: "border-cyan-500/30 bg-cyan-500/10 text-cyan-800 dark:text-cyan-200",
    },
    scripts_executables: {
      icon: AlertTriangle,
      label: t("pages.companySkills.includesScripts", { defaultValue: "Includes scripts" }),
      tooltip: t("pages.companySkills.includesScriptsDesc", { defaultValue: "Ships executable scripts. Review before installing." }),
...
function CompatChip({ compatibility }: { compatibility: CompanySkillCompatibility }) {
  ...
  const map = {
    ...
    invalid: {
      icon: XOctagon,
      label: t("pages.companySkills.invalid", { defaultValue: "Invalid" }),
      tooltip: t("pages.companySkills.invalidMarkdown", { defaultValue: "This skill cannot be installed — content is not valid Agent Skills markdown." }),
...
export function skillDetailBreadcrumbs(
  ...
  const trail = detail.folderId
    ...
    : [];
  return [
    { label: t("pages.companySkills.skills", { defaultValue: "Skills" }), href: "/skills" },
...
const DISCOVERY_SORT_LABELS: Record<DiscoverySort, string> = {
  agents: t("pages.companySkills.mostAgents", { defaultValue: "Most agents" }),
  stars: t("pages.companySkills.mostStars", { defaultValue: "Most stars" }),
  forks: t("pages.companySkills.mostForks", { defaultValue: "Most forks" }),
  recent: t("pages.companySkills.recentlyUpdated", { defaultValue: "Recently updated" }),
  alphabetical: t("pages.companySkills.alphabetical", { defaultValue: "Alphabetical" }),
};
...
function skillSettingsToastBody(skill: Pick<CompanySkillDetail, "categories" | "sharingScope">) {
  const sharing = skill.sharingScope === "private" ? t("pages.companySkills.sharingPrivate", { defaultValue: "Sharing: private" }) : t("pages.companySkills.sharingCompany", { defaultValue: "Sharing: company" });
  const categories = skill.categories.length ? `Categories: ${skill.categories.join(", ")}` : t("pages.companySkills.categoriesNone", { defaultValue: "Categories: none" });
...
function buildDiscoveryCards(
  ...
  for (const entry of catalog) {
    ...
    cards.push({
      ...
      name: entry.name,
      slug: entry.slug,
      author: entry.packageName ?? t("pages.companySkills.paperclip", { defaultValue: "Paperclip" }),
      ...
      updatedAt: 0,
      sourceBadge: "catalog",
      sourceLabel: entry.packageName ?? t("pages.companySkills.catalog", { defaultValue: "Catalog" }),
    });
  }
        ...
        <div className="min-w-0 flex-1">
          ...
          {badgeFolder !== undefined ? (
            <div className="mt-1 flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
              ...
              <span className="truncate">{badgeFolder ? badgeFolder.name : t("pages.companySkills.unfiled", { defaultValue: "Unfiled" })}</span>
            </div>
          ) : null}
        </div>
...
function CategoryNav({
  ...
  return (
    <nav className="flex flex-col gap-0.5 px-2">
      ...
      >
        <span>{t("pages.companySkills.all", { defaultValue: "All" })}</span>
...
export function DiscoveryGrid({
  ...
  /** Provision the caller's personal t("pages.companySkills.mySkills", { defaultValue: "My Skills" }) root on demand. */
    ...
    <div className="flex min-h-(--sz-calc-30) md:h-(--sz-calc-33) md:min-h-0 md:overflow-hidden">
      ...
      <aside className={cn("hidden w-60 shrink-0 flex-col overflow-hidden border-r border-border md:flex", showFolderRail && "md:hidden")}>
        <div className="border-b border-border px-4 py-4">
          <h2 className="text-sm font-semibold text-foreground">{t("pages.companySkills2.storeTitle", { defaultValue: "Skills Store" })}</h2>
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-border px-4 py-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm">
                <span className="text-muted-foreground">{t("pages.companySkills2.sort", { defaultValue: "Sort" })}</span>
          ...
          {availableSources.length > 1 ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm">
                  <span className="text-muted-foreground">{t("pages.companySkills2.source", { defaultValue: "Source" })}</span>
                  <span className="ml-1.5 capitalize">
                    {sourceBadgeFilter === "all" ? t("pages.companySkills.all", { defaultValue: "All" }) : sourceMeta(sourceBadgeFilter as CompanySkillSourceBadge, null).label}
                  </span>
                  <ChevronDown className="ml-1 h-3.5 w-3.5" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end">
                <DropdownMenuRadioGroup value={sourceBadgeFilter} onValueChange={setSourceBadgeFilter}>
                  <DropdownMenuRadioItem value="all">{t("pages.companySkills2.allSources", { defaultValue: "All sources" })}</DropdownMenuRadioItem>
          ...
          <Button
            ...
            title={t("pages.companySkills.scanWorkspaces", { defaultValue: "Scan project workspaces for skills" })}
          >
          ...
          {folderResult && onFolderSelect ? (
            <div className="w-full md:hidden">
              <FolderChip
                result={folderResult}
                selection={folderSelection}
                allLabel={t("pages.companySkills.allSkills", { defaultValue: "All skills" })}
          ...
          {onToggleSelectMode ? (
            <Button variant="ghost" size="sm" onClick={onToggleSelectMode}>
              {selectMode ? t("pages.companySkills.done", { defaultValue: "Done" }) : t("pages.companySkills.select", { defaultValue: "Select" })}
            </Button>
          ) : null}
        </div>
          ...
          <Tabs value={tab} onValueChange={(value) => onTabChange(value as DiscoveryTab)}>
            <TabsList variant="line" className="p-0">
              ...
              <TabsTrigger value="installed" className="px-3">
                <span>{t("pages.companySkills2.installed", { defaultValue: "Installed" })}</span>
              ...
              <TabsTrigger value="catalog" className="px-3">
                <span>{t("pages.companySkills.catalog", { defaultValue: "Catalog" })}</span>
              ...
              <TabsTrigger value="bundled" className="px-3">
                <span>{t("pages.companySkills.bundled", { defaultValue: "Bundled" })}</span>
          ...
          {loading ? (
            ...
            <div className="py-12">
              <EmptyState
                ...
                message={
                  totalCount === 0
                    ? t("pages.companySkills.noSkills", { defaultValue: "No skills yet. Create one or install from the catalog." })
                    : search || activeCategory || sourceFilterActive
                      ? t("pages.companySkills.noMatch", { defaultValue: "No skills match your filters." })
                      : t("pages.companySkills.noTabSkills", { defaultValue: "No skills in this tab yet." })
                }
              />
              {totalCount === 0 ? (
...
function NewSkillWizard({
  ...
  const steps = [t("pages.companySkills.basics", { defaultValue: "Basics" }), t("pages.companySkills.design", { defaultValue: "Design" }), t("pages.companySkills.content", { defaultValue: "Content" }), t("pages.companySkills.review", { defaultValue: "Review" })];
      ...
      {step === 0 ? (
        <div className="space-y-3">
          <Input
            ...
            placeholder={t("pages.companySkills.skillName", { defaultValue: "Skill name" })}
            className="h-9"
          />
          <Input
            value={draft.slug}
          ...
          <Textarea
            ...
            placeholder={t("pages.companySkills.taglineHint", { defaultValue: "One-line promise for the skill" })}
            className="min-h-20"
          />
        </div>
      ) : step === 1 ? (
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <SkillCardIcon
              ...
              card={{
                ...
                name: draft.name || t("pages.companySkills.newSkill", { defaultValue: "New Skill" }),
            ...
            <div className="min-w-0">
              <div className="truncate text-sm font-medium">{draft.name || t("pages.companySkills.newSkill", { defaultValue: "New Skill" })}</div>
              <div className="truncate text-xs text-muted-foreground">{draft.tagline || t("pages.companySkills.noTagline", { defaultValue: "No tagline yet." })}</div>
            </div>
          </div>
          <div>
            <label className="mb-2 block text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("pages.companySkills2.color", { defaultValue: "Color" })}</label>
          ...
          <div>
            <label className="mb-2 block text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("pages.companySkills2.categories", { defaultValue: "Categories" })}</label>
            <Input
              value={categoryDraft}
        ...
        <div className="space-y-4 text-sm">
          <div className="grid grid-cols-(--gtc-26) gap-y-2">
            <span className="text-muted-foreground">{t("pages.companySkills2.name", { defaultValue: "Name" })}</span>
            <span>{draft.name || t("pages.companySkills.untitled", { defaultValue: "Untitled" })}</span>
            <span className="text-muted-foreground">{t("pages.companySkills2.slug", { defaultValue: "Slug" })}</span>
            <span className="font-mono">{effectiveSlug || "skill"}</span>
            <span className="text-muted-foreground">{t("pages.companySkills2.scope", { defaultValue: "Scope" })}</span>
            <span>{draft.sharingScope === "private" ? t("pages.companySkills.private", { defaultValue: "Private" }) : t("pages.companySkills.company", { defaultValue: "Company" })}</span>
            <span className="text-muted-foreground">{t("pages.companySkills2.categories", { defaultValue: "Categories" })}</span>
            <span>{draft.categories.length ? draft.categories.join(", ") : "none"}</span>
          </div>
          <div className="space-y-2">
            <label className="block text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("pages.companySkills2.sharing", { defaultValue: "Sharing" })}</label>
            <div className="grid gap-2 sm:grid-cols-3">
              {(["company", "private"] as const).map((scope) => (
                ...
                >
                  <span className="block font-medium">{scope === "company" ? t("pages.companySkills.company", { defaultValue: "Company" }) : t("pages.companySkills.private", { defaultValue: "Private" })}</span>
                  <span className="mt-1 block text-xs text-muted-foreground">
                    {scope === "company" ? t("pages.companySkills.companyVisible", { defaultValue: "Visible inside this company." }) : t("pages.companySkills.privateVisible", { defaultValue: "Only visible in your library." })}
                  </span>
                </button>
              ))}
              <button
              ...
              >
                <span className="block font-medium">{t("pages.companySkills2.publicLink", { defaultValue: "Public link" })}</span>
                <span className="mt-1 block text-xs">{t("pages.companySkills2.comingLater", { defaultValue: "Coming later." })}</span>
              </button>
            </div>
          </div>
        </div>
  ...
  if (skill.compatibility === "invalid") {
    ...
    cta = (
      <Button onClick={onInstall} disabled={loadingPrimaryAction}>
        ...
        {loadingPrimaryAction ? t("pages.companySkills.preparing", { defaultValue: "Preparing..." }) : t("pages.companySkills.installInOrg", { defaultValue: "Install skill in this organization" })}
      </Button>
    );
    ...
    <div className="min-w-0">
      <div className="border-b border-border px-5 py-4">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
          <span className="uppercase tracking-(--tracking-caps)">{t("pages.companySkills2.key", { defaultValue: "Key" })}</span>
          <span className="font-mono">{skill.key}</span>
          <span className="uppercase tracking-(--tracking-caps)">·</span>
          <span className="uppercase tracking-(--tracking-caps)">{t("pages.companySkills2.hash", { defaultValue: "Hash" })}</span>
          ...
          <CopyText
            ...
            ariaLabel={t("pages.companySkills.copyHash", { defaultValue: "Copy content hash" })}
            title={t("pages.companySkills.copyHash", { defaultValue: "Copy content hash" })}
      ...
      <div className="min-h-(--sz-400px) px-5 py-5">
        {fileQuery.isLoading ? (
          ...
          <div className="text-sm text-destructive">{fileQuery.error instanceof Error ? fileQuery.error.message : t("pages.companySkills.loadFileFailed", { defaultValue: "Failed to load file" })}</div>
        ) : !fileQuery.data ? (
          <div className="text-sm text-muted-foreground">{t("pages.companySkills2.selectFile", { defaultValue: "Select a file to inspect." })}</div>
...
function InstallPreviewDialog({
  ...
  if (defaultAction === "update") {
    ...
    confirmLabel = t("pages.companySkills.replaceSkill", { defaultValue: "Replace existing skill" });
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => (!isPending ? onOpenChange(value) : null)}>
      <DialogContent className="sm:max-w-2xl" showCloseButton={!isPending}>
        <DialogHeader>
          <DialogTitle>
            {defaultAction === "update" ? t("pages.companySkills.update", { defaultValue: "Update" }) : defaultAction === "replace" ? t("pages.companySkills.replace", { defaultValue: "Replace" }) : t("pages.companySkills.install", { defaultValue: "Install" })} · {skill.name}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-4 text-sm">
          <div className="rounded-md border border-border p-3">
            <div className="grid grid-cols-(--gtc-26) gap-y-2 text-xs">
              <div className="text-muted-foreground">{t("pages.companySkills2.trust", { defaultValue: "Trust" })}</div>
              <div className="flex items-center gap-2">
                <TrustChip level={skill.trustLevel} />
                {skill.trustLevel === "markdown_only" ? (
                  <span className="text-muted-foreground">{t("pages.companySkills2.safe", { defaultValue: "Safe" })}</span>
                ) : skill.trustLevel === "scripts_executables" ? (
                  <span className="text-amber-800 dark:text-amber-200">{t("pages.companySkills2.reviewRequired", { defaultValue: "Review required" })}</span>
                ) : (
                  <span className="text-muted-foreground">{t("pages.companySkills2.nonScriptAssets", { defaultValue: "Non-script assets" })}</span>
                )}
              </div>
              <div className="text-muted-foreground">{t("pages.companySkills2.compatibility", { defaultValue: "Compatibility" })}</div>
              <div className="flex items-center gap-2">
              ...
              <div className="text-muted-foreground">{t("pages.companySkills2.requires", { defaultValue: "Requires" })}</div>
              <div className="text-foreground">{skill.requires.length === 0 ? "none" : skill.requires.join(", ")}</div>
              <div className="text-muted-foreground">{t("pages.companySkills2.roles", { defaultValue: "Roles" })}</div>
              <div className="text-foreground">{skill.recommendedForRoles.length === 0 ? "any" : skill.recommendedForRoles.join(" · ")}</div>
              <div className="text-muted-foreground">{t("pages.companySkills2.provenance", { defaultValue: "Provenance" })}</div>
              <div className="min-w-0">
          ...
          {advancedOpen ? (
            <div className="space-y-3 rounded-md border border-border p-3 text-xs">
              <div>
                <label className="mb-1 block uppercase tracking-wide text-muted-foreground">{t("pages.companySkills2.slugOverride", { defaultValue: "Slug override" })}</label>
              ...
              <label className="flex items-center gap-2">
                ...
                <span>{t("pages.companySkills2.forceReplace", { defaultValue: "Force replace existing same-key skill" })}</span>
              </label>
            </div>
          ) : null}
...
function AttachAgentsPopover({
  ...
  return (
    <AgentMultiSelect
      ...
      pending={pending}
      triggerLabel={t("pages.companySkills.addToAgent", { defaultValue: "Add to agent" })}
      ...
      headerContent={sortedVersions.length > 0 ? (
        <div className="mt-2 flex items-center gap-2 text-xs">
          <span className="shrink-0 text-muted-foreground">{t("pages.companySkills2.version", { defaultValue: "Version" })}</span>
          <select
      ...
      emptyMessage={eligible.length === 0 ? t("pages.companySkills.noSkillAgents", { defaultValue: "No agents in this company support skills yet." }) : t("pages.companySkills.noAgents", { defaultValue: "No agents yet." })}
...
const SKILL_DETAIL_TABS: Array<{ value: SkillDetailTab; label: string; icon: typeof FileText }> = [
  { value: "overview", label: t("pages.companySkills.overview", { defaultValue: "Overview" }), icon: FileText },
  { value: "files", label: t("pages.companySkills.files", { defaultValue: "Files" }), icon: FolderOpen },
  { value: "versions", label: t("pages.companySkills.versions", { defaultValue: "Versions" }), icon: History },
  { value: "agents", label: t("pages.companySkills.agents", { defaultValue: "Agents" }), icon: Users },
];
      ...
      <DialogContent className="flex max-h-(--sz-85vh) w-full !max-w-(--pct-90) flex-col overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <DialogHeader className="shrink-0">
            <DialogTitle>{t("pages.companySkills2.diffFiles", { defaultValue: "Diff · skill files" })}</DialogTitle>
          </DialogHeader>
          <div className="flex flex-wrap items-center gap-3 text-xs">
            <label className="flex items-center gap-2">
              <Badge variant="outline" className="border-red-500/30 bg-red-500/10 uppercase tracking-wider text-red-400">{t("pages.companySkills2.old", { defaultValue: "Old" })}</Badge>
              <select
                value={leftVersionId ?? ""}
              ...
              >
                <option value="">{t("pages.companySkills2.initial", { defaultValue: "Initial" })}</option>
                {sorted.map((version) => (
            ...
            <label className="flex items-center gap-2">
              <Badge variant="outline" className="border-green-500/30 bg-green-500/10 uppercase tracking-wider text-green-400">{t("pages.companySkills2.new", { defaultValue: "New" })}</Badge>
              <select
                value={right?.id ?? ""}
        ...
        <div className="flex min-h-0 flex-1 gap-3">
          ...
          <div className="min-w-0 flex-1 overflow-auto rounded-md border border-border text-xs">
            {!right ? (
              <div className="p-6 text-center text-sm text-muted-foreground">{t("pages.companySkills2.selectVersion", { defaultValue: "Select a version to compare." })}</div>
            ) : left?.id === right.id ? (
              <div className="p-6 text-center text-sm text-muted-foreground">{t("pages.companySkills2.sameVersion", { defaultValue: "Both sides are the same version." })}</div>
            ) : (
              <div className="font-mono text-xs leading-6">
                <div className="grid grid-cols-(--gtc-1) border-b border-border/60 bg-muted/30 px-3 py-2 text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">
                  <span>{t("pages.companySkills2.old", { defaultValue: "Old" })}</span>
                  <span>{t("pages.companySkills2.new", { defaultValue: "New" })}</span>
                  <span />
...
function SkillLocationCard({
  ...
  const canonical = folderPath && folderPath.length > 0 ? folderPath : t("pages.companySkills.unfiled", { defaultValue: "Unfiled" });
  return (
    <section>
      <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("pages.companySkills2.location", { defaultValue: "Location" })}</div>
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        >
          ...
          {copied ? t("pages.companySkills.copied", { defaultValue: "Copied" }) : t("pages.companySkills.copyPath", { defaultValue: "Copy path" })}
        </Button>
        {onMove ? (
  ...
  return (
    <section>
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        {categories.length === 0 ? (
          <span className="text-xs text-muted-foreground">{t("pages.companySkills2.noTags", { defaultValue: "No tags yet." })}</span>
        ) : null}
      </div>
      <Input
        value={draft}
        ...
        placeholder={t("pages.companySkills.addTag", { defaultValue: "Add a tag…" })}
        disabled={pending}
...
export function SkillDetailPage({
  ...
  // Top-level description is clamped to four lines; t("pages.companySkills.viewAll", { defaultValue: "View all" }) expands it. We
  ...
  const githubLabel = githubSource
    ? githubSource.hostname === "github.com"
      ? t("pages.companySkills.gitHub", { defaultValue: "GitHub" })
    ...
    return (
      <div className="grid min-h-(--sz-560px) gap-0 lg:grid-cols-(--gtc-28)">
        <aside className="border-b border-border pb-3 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-3">
          <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("pages.companySkills.files", { defaultValue: "Files" })}</div>
          <SkillTree
          ...
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {skill.editable && file?.editable ? (
                editMode ? (
                  <>
                    <Button variant="ghost" size="sm" onClick={() => setEditMode(false)} disabled={savePending}>{t("pages.companySkills2.cancel", { defaultValue: "Cancel" })}</Button>
  ...
  function renderOverviewBody() {
    return (
      <div className="space-y-6">
        <section>
          <h2 className="mb-2 text-sm font-medium">{t("pages.companySkills2.about", { defaultValue: "About" })}</h2>
          {fileLoading ? (
            <PageSkeleton variant="detail" />
          ) : file?.markdown ? (
            <MarkdownBody softBreaks={false} linkIssueReferences={false}>{body || skill.description || t("pages.companySkills.noOverview", { defaultValue: "No overview yet." })}</MarkdownBody>
          ) : (
            <p className="text-sm text-muted-foreground">{skill.description ?? t("pages.companySkills.noOverview", { defaultValue: "No overview yet." })}</p>
          )}
        </section>
        <section className="grid min-w-0 gap-3 text-sm sm:grid-cols-2">
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("pages.companySkills2.key", { defaultValue: "Key" })}</div>
            <div className="mt-1 truncate font-mono">{skill.key}</div>
          </div>
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("pages.companySkills2.source", { defaultValue: "Source" })}</div>
            <div className="mt-1 min-w-0 [overflow-wrap:anywhere]">{sourceLocatorText ?? source.label}</div>
          </div>
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("pages.companySkills2.version", { defaultValue: "Version" })}</div>
            <div className="mt-1">{versionLabel(skill.currentVersion ?? null)}</div>
          </div>
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("pages.companySkills2.mode", { defaultValue: "Mode" })}</div>
            <div className="mt-1 flex flex-wrap items-center gap-2">
              {skill.editable ? (
                t("pages.companySkills.editable", { defaultValue: "Editable" })
              ) : (
                <>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <div className="text-sm text-muted-foreground">
            {versionsLoading ? t("pages.companySkills.loadingVersions", { defaultValue: "Loading versions..." }) : `${versions.length} ${versions.length === 1 ? "version" : "versions"}`}
          </div>
          <Button
            type="button"
            variant="outline"
        ...
        <div className="border-y border-border">
          {versionsLoading ? (
            ...
            <div className="py-6 text-sm text-muted-foreground">{t("pages.companySkills2.noVersions", { defaultValue: "No saved versions yet." })}</div>
          ) : (
            sortedVersions.map((version) => (
  ...
  function renderAgentsBody() {
    ...
    // multi-selector behind t("pages.companySkills.addToAgent", { defaultValue: "Add to agent" }) is where you attach more.
    ...
    <div className="min-h-(--sz-calc-30)">
      <div className="border-b border-border px-4 py-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {/* GitHub-style social proof, top-right: installs · stars · fork.
              t("pages.companySkills.installs", { defaultValue: "Installs" }) counts agents that currently have this skill attached
          ...
          <div className="flex flex-wrap items-center justify-end gap-1">
            ...
            <div className="flex items-center overflow-hidden rounded-md border border-border">
              <Tooltip>
                ...
                <TooltipContent>{t("pages.companySkills2.agentsHint", { defaultValue: "Agents in this company that currently have this skill installed." })}</TooltipContent>
              </Tooltip>
              <button
                type="button"
                ...
                title={detail.starredByCurrentActor ? t("pages.companySkills.unstarSkill", { defaultValue: "Unstar this skill" }) : t("pages.companySkills.starSkill", { defaultValue: "Star this skill" })}
              >
              ...
              <button
                ...
                title={t("pages.companySkills.forkSkill", { defaultValue: "Fork this skill" })}
              >
                <GitFork className="h-3.5 w-3.5" />
                <span className="hidden sm:inline">{t("pages.companySkills2.fork", { defaultValue: "Fork" })}</span>
      ...
      <div className="grid gap-6 px-4 py-4 xl:grid-cols-(--gtc-29)">
        ...
        <aside className="min-w-0 space-y-6 border-t border-border pt-4 xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0">
          ...
          />
          <section>
            <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("pages.companySkills.agents", { defaultValue: "Agents" })}</div>
            <div className="space-y-3">
              ...
              {detail.usedByAgents.length === 0 ? (
                <p className="text-xs text-muted-foreground">{t("pages.companySkills2.noAgents", { defaultValue: "No agents attached yet." })}</p>
              ) : (
          ...
          {/* Provenance: where this skill came from, with org/path linked when
              ...
              (PAP-10907). */}
          <section>
            <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("pages.companySkills2.source", { defaultValue: "Source" })}</div>
            {githubSource ? (
          ...
          {detail.sourceType === "github" ? (
            <section>
              <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("pages.companySkills2.updates", { defaultValue: "Updates" })}</div>
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Pin className="h-3.5 w-3.5 shrink-0" aria-label={t("pages.companySkills.pinnedRevision", { defaultValue: "Pinned source revision" })} />
                    </TooltipTrigger>
                    <TooltipContent>{t("pages.companySkills.pinnedRevision", { defaultValue: "Pinned source revision" })}</TooltipContent>
                  </Tooltip>
          ...
          <section>
            ...
            >
              ...
              <span className="flex-1">{t("pages.companySkills2.settings", { defaultValue: "Settings" })}</span>
            </button>
          </section>
        </aside>
      </div>
      ...
      {isDirty ? (
        <div className="fixed bottom-6 left-1/2 z-40 flex -translate-x-1/2 items-center gap-3 rounded-full border border-border bg-background/95 px-4 py-2 shadow-lg backdrop-blur">
          <span className="text-sm text-muted-foreground">{t("pages.companySkills2.unsavedChanges", { defaultValue: "Unsaved changes" })}</span>
          <Button
            variant="ghost"
            size="sm"
            onClick={() => {
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-5">
            <div className="space-y-1.5">
              <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("pages.companySkills2.categories", { defaultValue: "Categories" })}</label>
              <Input
              ...
              />
              <p className="text-xs text-muted-foreground">{t("pages.companySkills2.categoriesHint", { defaultValue: "Separate categories with commas. Leave empty to clear categories." })}</p>
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("pages.companySkills2.sharing", { defaultValue: "Sharing" })}</label>
              <select
              ...
              >
                <option value="company">{t("pages.companySkills2.companyVisible", { defaultValue: "Company — visible inside this company" })}</option>
                <option value="private">{t("pages.companySkills2.privateVisible", { defaultValue: "Private — only visible in your library" })}</option>
              </select>
              <p className="text-xs text-muted-foreground">{t("pages.companySkills2.publicLater", { defaultValue: "Public link sharing is coming later." })}</p>
            </div>
            <div className="flex justify-end gap-2 border-t border-border pt-4">
              ...
              >
                <Save className="mr-1.5 h-3.5 w-3.5" />
                {updateSettingsPending ? t("pages.companySkills.saving2", { defaultValue: "Saving…" }) : t("pages.companySkills.saveSettings", { defaultValue: "Save settings" })}
              </Button>
            </div>
            {detail.editable ? (
              <div className="rounded-md border border-destructive/40 p-3">
                <div className="text-xs font-semibold uppercase tracking-wide text-destructive">{t("pages.companySkills2.dangerZone", { defaultValue: "Danger zone" })}</div>
                <div className="mt-2 flex items-center justify-between gap-3">
                  <p className="min-w-0 text-xs text-muted-foreground">{t("pages.companySkills2.removeHint", { defaultValue: "Remove this skill from the company library." })}</p>
                  <Button
                    variant="destructive"
                    size="sm"
                    className="shrink-0"
                    ...
                    title={detail.usedByAgents.length > 0 ? t("pages.companySkills.detachFirst", { defaultValue: "Detach this skill from all agents before removing it." }) : undefined}
                  >
...
function SkillPane({
  ...
  if (!detail) {
    ...
    return (
      <EmptyState
        icon={Boxes}
        message={t("pages.companySkills.selectSkillFiles", { defaultValue: "Select a skill to inspect its files." })}
      />
    );
  }
  ...
  const removeDisabledReason = removeBlocked
    ? t("pages.companySkills.detachFirst", { defaultValue: "Detach this skill from all agents before removing it." })
    : null;
      ...
      <div className="border-b border-border px-5 py-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            >
              <Trash2 className="mr-1.5 h-3.5 w-3.5" />
              {deletePending ? t("pages.companySkills.removing2", { defaultValue: "Removing..." }) : t("pages.companySkills.remove", { defaultValue: "Remove" })}
            </Button>
            {detail.editable ? (
              <button
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            <div className="flex min-w-0 items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("pages.companySkills2.source", { defaultValue: "Source" })}</span>
            ...
            {detail.sourceType === "github" && (
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("pages.companySkills2.pin", { defaultValue: "Pin" })}</span>
                ...
                {updateStatus?.supported && !updateStatus.hasUpdate && !updateStatusLoading && (
                  <span className="text-xs text-muted-foreground">{t("pages.companySkills2.upToDate2", { defaultValue: "Up to date" })}</span>
                )}
            ...
            <div className="flex items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("pages.companySkills2.key", { defaultValue: "Key" })}</span>
              <span className="font-mono text-xs">{detail.key}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("pages.companySkills2.mode", { defaultValue: "Mode" })}</span>
              <span>{detail.editable ? t("pages.companySkills.editable", { defaultValue: "Editable" }) : t("pages.companySkills.readOnly", { defaultValue: "Read only" })}</span>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("pages.companySkills2.trust", { defaultValue: "Trust" })}</span>
          ...
          <div className="flex flex-wrap items-start gap-x-3 gap-y-2">
            <div className="flex items-center gap-2">
              <span className="text-(length:--text-micro) uppercase tracking-(--tracking-caps) text-muted-foreground">{t("pages.companySkills2.usedBy", { defaultValue: "Used by" })}</span>
              <AttachAgentsPopover
                agents={attachAgents}
            ...
            {usedBy.length === 0 ? (
              <span className="text-muted-foreground">{t("pages.companySkills2.noAgentsAttached", { defaultValue: "No agents attached" })}</span>
            ) : (
...
export function CompanySkills() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("pages.companySkills.skills", { defaultValue: "Skills" }), href: "/skills" },
      ...(isStudioNew
        ? [{ label: studioForkFromId ? t("pages.companySkills.forkSkillShort", { defaultValue: "Fork skill" }) : t("pages.companySkills.newSkillShort", { defaultValue: "New skill" }) }]
        : activeDetail
          ? skillDetailBreadcrumbs(activeDetail, skillFoldersQuery.data).slice(1)
          : routeSkillToken
            ? [{ label: t("pages.companySkills.detail", { defaultValue: "Detail" }) }]
            : []),
    ]);
  ...
  const importSkill = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("pages.companySkills.skillsImported", { defaultValue: "Skills imported" }),
        body: `${result.imported.length} skill${result.imported.length === 1 ? "" : "s"} added.`,
      });
      if (result.warnings[0]) {
        pushToast({ tone: "warn", title: t("pages.companySkills.importWarnings", { defaultValue: "Import warnings" }), body: result.warnings[0] });
      }
      setSource("");
    },
    onError: (error) => {
      reportSkillError(error, t("pages.companySkills.importFailed", { defaultValue: "Skill import failed" }), t("pages.companySkills.importSourceFailed", { defaultValue: "Failed to import skill source." }), t("pages.companySkills.importing", { defaultValue: "Importing skills" }));
    },
  });
  ...
  const scanProjects = useMutation({
    ...
    onMutate: () => {
      setScanStatusMessage(t("pages.companySkills.scanning", { defaultValue: "Scanning project workspaces for skills..." }));
    },
    onSuccess: async (result) => {
      setScanStatusMessage(t("pages.companySkills.refreshing", { defaultValue: "Refreshing skills list..." }));
      await queryClient.invalidateQueries({ queryKey: queryKeys.companySkills.list(selectedCompanyId!) });
      const summary = formatProjectScanSummary(result);
      setScanStatusMessage(summary);
      pushToast({
        tone: "success",
        title: t("pages.companySkills.scanComplete", { defaultValue: "Project skill scan complete" }),
        body: summary,
      });
      if (result.conflicts[0]) {
        pushToast({
          tone: "warn",
          title: t("pages.companySkills.conflictsFound", { defaultValue: "Skill conflicts found" }),
          body: result.conflicts[0].reason,
        });
      } else if (result.warnings[0]) {
        pushToast({
          tone: "warn",
          title: t("pages.companySkills.scanWarnings", { defaultValue: "Scan warnings" }),
    ...
    onError: (error) => {
      ...
      reportSkillError(error, t("pages.companySkills.scanFailed", { defaultValue: "Project skill scan failed" }), t("pages.companySkills.scanWorkspacesFailed", { defaultValue: "Failed to scan project workspaces." }), t("pages.companySkills.scanningProjects", { defaultValue: "Scanning projects for skills" }));
    },
  });
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: skill.forkedFromSkillId ? t("pages.companySkills.forkCreated", { defaultValue: "Skill fork created" }) : t("pages.companySkills.skillCreated", { defaultValue: "Skill created" }),
        body: `${skill.name} is now editable in the Paperclip workspace.`,
      });
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : t("pages.companySkills.createFailed", { defaultValue: "Failed to create skill." });
      setCreateError(message);
      reportSkillError(error, t("pages.companySkills.creationFailed", { defaultValue: "Skill creation failed" }), t("pages.companySkills.createFailed", { defaultValue: "Failed to create skill." }), t("pages.companySkills.creatingSkill", { defaultValue: "Creating a skill" }));
    },
  });
  ...
  const saveFile = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("pages.companySkills.skillSaved", { defaultValue: "Skill saved" }),
        body: result.path,
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("pages.companySkills.saveFailed", { defaultValue: "Save failed" }),
        body: error instanceof Error ? error.message : t("pages.companySkills.saveFileFailed", { defaultValue: "Failed to save skill file." }),
      });
    },
  });
  ...
  const toggleStar = useMutation({
    mutationFn: () => {
      if (!activeDetail) throw new Error(t("pages.companySkills.selectFirst", { defaultValue: "Select a skill first." }));
  ...
  const updateSkillSettings = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({ tone: "success", title: t("pages.companySkills.settingsUpdated", { defaultValue: "Skill settings updated" }), body: skillSettingsToastBody(skill) });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("pages.companySkills.settingsUpdateFailed", { defaultValue: "Skill settings update failed" }),
        body: error instanceof Error ? error.message : t("pages.companySkills.settingsUpdateFailed2", { defaultValue: "Failed to update skill settings." }),
      });
    },
  });
  ...
  const installUpdate = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("pages.companySkills.skillUpdated", { defaultValue: "Skill updated" }),
        body: skill.sourceRef ? `Pinned to ${shortRef(skill.sourceRef)}` : skill.name,
      });
    },
    onError: (error) => {
      reportSkillError(error, t("pages.companySkills.updateFailed", { defaultValue: "Update failed" }), t("pages.companySkills.installUpdateFailed", { defaultValue: "Failed to install skill update." }), t("pages.companySkills.updatingSkill", { defaultValue: "Updating this skill" }));
    },
  });
  ...
  const installCatalog = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: result.action === "created" ? t("pages.companySkills.skillInstalled", { defaultValue: "Skill installed" }) : result.action === "updated" ? t("pages.companySkills.skillUpdated", { defaultValue: "Skill updated" }) : t("pages.companySkills.skillUpToDate", { defaultValue: "Skill is up to date" }),
    ...
    onError: (error) => {
      const message = error instanceof Error ? error.message : t("pages.companySkills.catalogInstallFailed", { defaultValue: "Failed to install catalog skill." });
      ...
      policyDenial.capture(error, t("pages.companySkills.installingSkill", { defaultValue: "Installing this skill" }));
    },
  });
    ...
    onSuccess: async (folder) => {
      ...
      if (moveAfterCreateSkillIds.length > 0) {
        ...
        try {
          ...
          pushToast({
            tone: "error",
            title: t("pages.companySkills2.folderMoveFailed", { defaultValue: "Folder created, move failed" }),
            body: moveError instanceof Error ? moveError.message : t("pages.companySkills2.moveSkillsFailed", { defaultValue: "Failed to move the selected skills." }),
          });
          return;
        }
      } else {
      ...
      pushToast({ tone: "success", title: t("pages.companySkills2.folderCreated", { defaultValue: "Folder created" }), body: folder.name });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("pages.companySkills2.folderSaveFailed", { defaultValue: "Folder save failed" }),
        body: error instanceof Error ? error.message : t("pages.companySkills2.saveFolderFailed", { defaultValue: "Failed to save folder." }),
      });
    },
  });
  const updateFolder = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("pages.companySkills2.folderSaveFailed", { defaultValue: "Folder save failed" }),
        body: error instanceof Error ? error.message : t("pages.companySkills2.updateFolderFailed", { defaultValue: "Failed to update folder." }),
      });
    },
  });
  const moveFolder = useMutation({
    ...
    onSuccess: async (folder) => {
      ...
      pushToast({
        tone: "success",
        title: t("pages.companySkills2.folderMoved", { defaultValue: "Folder moved" }),
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("pages.companySkills2.folderMoveFailed2", { defaultValue: "Folder move failed" }),
        body: error instanceof Error ? error.message : t("pages.companySkills2.moveFolderFailed", { defaultValue: "Failed to move folder." }),
      });
    },
  });
  const deleteFolder = useMutation({
    ...
    onSuccess: async (_, folderId) => {
      ...
      pushToast({ tone: "success", title: t("pages.companySkills2.folderDeleted", { defaultValue: "Folder deleted" }), body: t("pages.companySkills2.skillsToUnfiled", { defaultValue: "Skills moved to Unfiled." }) });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("pages.companySkills2.folderDeleteFailed", { defaultValue: "Folder delete failed" }),
        body: error instanceof Error ? error.message : t("pages.companySkills2.deleteFolderFailed", { defaultValue: "Failed to delete folder." }),
      });
    },
  });
  const moveSkillToFolder = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("pages.companySkills2.moveFailed", { defaultValue: "Move failed" }),
        body: error instanceof Error ? error.message : t("pages.companySkills2.moveSkillFailed", { defaultValue: "Failed to move skill." }),
      });
    },
  });
  ...
  async function moveSelectedSkills(folderId: string | null) {
    ...
    try {
      ...
      pushToast({ tone: "success", title: t("pages.companySkills2.skillsMoved", { defaultValue: "Skills moved" }), body: `${ids.length} skill${ids.length === 1 ? "" : "s"} filed.` });
    } catch (moveError) {
      pushToast({
        tone: "error",
        title: t("pages.companySkills2.moveSkillsFailed2", { defaultValue: "Failed to move skills" }),
        body: moveError instanceof Error ? moveError.message : t("pages.companySkills2.moveSkillsFailed", { defaultValue: "Failed to move the selected skills." }),
      });
    }
  }

  // Provision the signed-in user's personal t("pages.companySkills.mySkills", { defaultValue: "My Skills" }) root, then select it.
  const ensureMyFolder = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("pages.companySkills2.openMySkillsFailed", { defaultValue: "Couldn't open My Skills" }),
        body: error instanceof Error ? error.message : t("pages.companySkills2.personalFolderFailed", { defaultValue: "Failed to create your personal folder." }),
      });
    },
  });
  ...
  // Inline folder creation used by the move dialog's t("pages.companySkills2.newFolderInside", { defaultValue: "New folder inside…" }) affordance.
  async function createFolderInline(parentId: string | null, name: string): Promise<string | null> {
    try {
      ...
      pushToast({
        tone: "error",
        title: t("pages.companySkills2.folderCreateFailed", { defaultValue: "Folder create failed" }),
        body: error instanceof Error ? error.message : t("pages.companySkills2.createFolderFailed", { defaultValue: "Failed to create folder." }),
      });
      return null;
    }
  }
  ...
  async function performDialogMove(folderId: string | null) {
    ...
    try {
      ...
      pushToast({
        tone: "success",
        title: ids.length === 1 ? t("pages.companySkills2.skillMoved", { defaultValue: "Skill moved" }) : t("pages.companySkills2.skillsMoved", { defaultValue: "Skills moved" }),
        body: folderId
          ? `Filed under ${skillFolderResult?.folders.find((folder) => folder.id === folderId)?.name ?? "folder"}.`
          : t("pages.companySkills2.movedToUnfiled", { defaultValue: "Moved to Unfiled." }),
      });
    } catch (moveError) {
      pushToast({
        tone: "error",
        title: t("pages.companySkills2.moveFailed", { defaultValue: "Move failed" }),
        body: moveError instanceof Error ? moveError.message : t("pages.companySkills2.moveFailed2", { defaultValue: "Failed to move." }),
      });
    }
  }
  ...
  async function handleAttachSubmit(nextAgentIds: string[], versionId: string | null = null) {
    ...
    try {
      ...
      pushToast({ tone: "success", title: t("pages.companySkills2.agentsUpdated", { defaultValue: "Agents updated" }), body: `${nextAgentIds.length} agent(s) attached.` });
    } catch (error) {
      pushToast({ tone: "error", title: t("pages.companySkills.updateFailed", { defaultValue: "Update failed" }), body: error instanceof Error ? error.message : t("pages.companySkills2.agentSkillsFailed", { defaultValue: "Failed to update agent skills." }) });
    }
  }
  ...
  const deleteSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("pages.companySkills2.skillRemoved", { defaultValue: "Skill removed" }),
        body: `${skill.name} was removed from the company skill library.`,
      });
    },
    onError: (error) => {
      reportSkillError(error, t("pages.companySkills2.removeFailed", { defaultValue: "Remove failed" }), t("pages.companySkills2.removeSkillFailed", { defaultValue: "Failed to remove skill." }), t("pages.companySkills2.removingSkill", { defaultValue: "Removing this skill" }));
    },
  });
  ...
  // t("pages.companySkills.backToStore", { defaultValue: "Back to store" }) returns to the discovery grid while keeping the tab /
  ...
  const studioTitle = studioForkFromId ? t("pages.companySkills.forkSkillShort", { defaultValue: "Fork skill" }) : t("pages.companySkills.createNewSkill", { defaultValue: "Create a new skill" });
  const studioDescription = studioForkFromId
    ? t("pages.companySkills.forkMetaHint", { defaultValue: "Review the fork metadata and create an editable company copy." })
    : t("pages.companySkills.editableCopyHint", { defaultValue: "Create an editable company skill in the Paperclip workspace." });
  return (
    <>
      {policyDenial.denial ? (
          ...
          <div className="space-y-3 text-sm">
            <p>
              {deleteTargetDetail
                ...
                : t("pages.companySkills.removeConfirm", { defaultValue: "You are about to remove this skill." })}
            </p>
          ...
          <DialogFooter>
            {(deleteTargetDetail?.usedByAgents.length ?? 0) > 0 ? (
              ...
              <>
                ...
                >
                  {deleteSkill.isPending ? t("pages.companySkills.removing2", { defaultValue: "Removing..." }) : t("pages.companySkills.removeSkill", { defaultValue: "Remove skill" })}
                </Button>
              </>
            )}
          </DialogFooter>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 border-b border-border pb-2">
              <Input
                ...
                placeholder={t("pages.companySkills.importPlaceholder", { defaultValue: "Paste path, GitHub URL, or skills.sh command" })}
            ...
            >
              <span>
                <span className="block font-medium">{t("pages.companySkills2.browseSkillsSh", { defaultValue: "Browse skills.sh" })}</span>
                <span className="mt-1 block text-muted-foreground">{t("pages.companySkills2.installCommandsHint", { defaultValue: "Find install commands and paste one here." })}</span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
            <a
            ...
            >
              <span>
                <span className="block font-medium">{t("pages.companySkills2.searchGitHub", { defaultValue: "Search GitHub" })}</span>
                <span className="mt-1 block text-muted-foreground">{t("pages.companySkills2.githubHint", { defaultValue: "Look for repositories with `SKILL.md`, then paste the repo URL." })}</span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
          </div>
      ...
      <MobileFolderSheet
        ...
        selection={folderSelection}
        allLabel={t("pages.companySkills.allSkills", { defaultValue: "All skills" })}
        itemLabelPlural={t("pages.companySkills.skills", { defaultValue: "Skills" })}
      ...
      <MoveToFolderDialog
        ...
        result={railSkillFolderResult}
        title={moveDialog?.title ?? t("pages.companySkills.moveToFolder", { defaultValue: "Move to folder" })}
        ...
        <DiscoveryGrid
          ...
          onOpenMoveCard={showInstalledFolders ? (card) => {
            ...
            setMoveDialog({
              ...
              subtitle: t("pages.companySkills.chooseFolder", { defaultValue: "Choose a destination folder." }),
          ...
          onMoveCard={showInstalledFolders ? (card, folderId) => {
            ...
            pushToast({
              tone: "success",
              title: t("pages.companySkills2.skillMoved", { defaultValue: "Skill moved" }),
              body: folderId
                ? `Moved "${card.name}" to ${skillFolderResult?.folders.find((folder) => folder.id === folderId)?.name ?? "folder"}.`
                : `Moved "${card.name}" to Unfiled.`,
              action: {
                label: t("pages.companySkills.undo", { defaultValue: "Undo" }),
