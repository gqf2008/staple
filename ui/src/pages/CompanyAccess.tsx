...
export function CompanyAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("pages.companyAccess.company", { defaultValue: "Company" }), href: "/dashboard" },
      { label: t("pages.companyAccess.settings", { defaultValue: "Settings" }), href: "/company/settings" },
      { label: t("pages.companyAccess.members", { defaultValue: "Members" }) },
    ]);
  ...
  const updateMemberMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({
        title: t("pages.companyAccess.memberUpdated", { defaultValue: "Member updated" }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("pages.companyAccess.memberUpdateFailed", { defaultValue: "Failed to update member" }),
  ...
  const approveJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("pages.companyAccess.joinApproved", { defaultValue: "Join request approved" }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("pages.companyAccess.joinApproveFailed", { defaultValue: "Failed to approve join request" }),
        body: error instanceof Error ? error.message : t("pages.companyAccess.unknownError", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  ...
  const rejectJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("pages.companyAccess.joinRejected", { defaultValue: "Join request rejected" }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("pages.companyAccess.joinRejectFailed", { defaultValue: "Failed to reject join request" }),
        body: error instanceof Error ? error.message : t("pages.companyAccess.unknownError", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  ...
  const archiveMemberMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        title: t("pages.companyAccess.memberRemoved", { defaultValue: "Member removed" }),
        body:
    ...
    onError: (error) => {
      pushToast({
        title: t("pages.companyAccess.memberRemoveFailed", { defaultValue: "Failed to remove member" }),
        body: error instanceof Error ? error.message : t("pages.companyAccess.unknownError", { defaultValue: "Unknown error" }),
        tone: "error",
      });
    },
  ...
  if (membersQuery.error) {
    const message =
      membersQuery.error instanceof ApiError && membersQuery.error.status === 403
        ? t("pages.companyAccess.noPermission", { defaultValue: "You do not have permission to manage company members." })
        : membersQuery.error instanceof Error
          ? membersQuery.error.message
          : t("pages.companyAccess.loadFailed", { defaultValue: "Failed to load company members." });
    ...
    <div className="max-w-6xl space-y-8">
      ...
      <section className="space-y-4">
              ...
              {pendingHumanJoinRequests.map((request) => (
                <PendingJoinRequestCard
                  ...
                  title={
                    ...
                    t("pages.companyAccess.unknownRequester", { defaultValue: "Unknown human requester" })
                  }
                  subtitle={
                    request.requesterUser?.email ||
                    request.requestEmailSnapshot ||
                    request.requestingUserId ||
                    t("pages.companyAccess.noEmail", { defaultValue: "No email available" })
                  }
                  context={
                    request.invite
                      ? `${request.invite.allowedJoinTypes} join invite${request.invite.humanRole ? ` • default role ${request.invite.humanRole}` : ""}`
                      : t("pages.companyAccess.noInviteMeta", { defaultValue: "Invite metadata unavailable" })
                  }
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          ...
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">{t("pages.companyAccess.noMembers", { defaultValue: "No user memberships found for this company yet." })}</div>
          ) : (
            members.map((member) => {
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <label className="space-y-2 text-sm">
                  ...
                  >
                    <option value="active">{t("pages.companyAccess.active", { defaultValue: "Active" })}</option>
                    <option value="pending">{t("pages.companyAccess.pending", { defaultValue: "Pending" })}</option>
                    <option value="suspended">{t("pages.companyAccess.suspended", { defaultValue: "Suspended" })}</option>
                  </select>
                </label>
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            >
              {updateMemberMutation.isPending ? t("pages.companyAccess.saving", { defaultValue: "Saving…" }) : t("pages.companyAccess.saveMember", { defaultValue: "Save member" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!removingMember} onOpenChange={(open) => !open && setRemovingMemberId(null)}>
        <DialogContent className="max-w-xl">
              ...
              <div className="rounded-lg border border-border px-3 py-3">
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ? t("pages.companyAccess.checkingTasks", { defaultValue: "Checking assigned tasks..." })
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    {activeReassignmentAgents.length > 0 ? (
                      <optgroup label={t("pages.companyAccess.agents", { defaultValue: "Agents" })}>
          ...
          <DialogFooter>
            ...
            >
              {archiveMemberMutation.isPending ? t("pages.companyAccess.removing", { defaultValue: "Removing..." }) : t("pages.companyAccess.removeMember", { defaultValue: "Remove member" })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
export function CompanyAccessLegacyRoute() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("pages.companyAccess.settings", { defaultValue: "Settings" }), href: "/company/settings" },
      { label: t("pages.companyAccess.access", { defaultValue: "Access" }) },
    ]);
  }, [setBreadcrumbs]);
  ...
  if (isLoading) {
    return <div className="text-sm text-muted-foreground">{t("pages.companyAccess.checkingPermissions", { defaultValue: "Checking for advanced permission extensions..." })}</div>;
  }
