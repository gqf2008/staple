  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        ...
        <CardDescription>
          {t("pages.costs.financeLedgerDesc", { defaultValue: "Account-level charges that do not map to a single inference request." })}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={t("pages.costs.totalEvents", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("pages.costs.credits", { defaultValue: "Credits" })}
          value={formatCents(creditCents)}
          subtitle={t("pages.costs.refunds", { defaultValue: "Refunds, offsets, and credit returns" })}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("pages.costs.net", { defaultValue: "Net" })}
          value={formatCents(netCents)}
          subtitle={t("pages.costs.netDesc", { defaultValue: "Debit minus credit for the selected period" })}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("pages.costs.estimated", { defaultValue: "Estimated" })}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("pages.costs.estimatedDesc", { defaultValue: "Estimated debits that are not yet invoice-authoritative" })}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("pages.costs.title", { defaultValue: "Costs" }) }]);
  }, [setBreadcrumbs]);
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("pages.secrets.allProviders")}</span>
    ...
    return [
      {
        ...
        label: (
          <span className="flex items-center gap-1.5">
            <span>{t("ui.pages.costs.all-billers")}</span>
            {billerKeys.length > 0 ? (
              <>
                <span className="font-mono text-xs text-muted-foreground">{formatTokens(allTokens)}</span>
                <span className="text-xs text-muted-foreground">{formatCents(allCents)}</span>
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("pages.costs.title", { defaultValue: "Costs" })}</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("pages.costs.subtitle", { defaultValue: "Inference spend, platform fees, credits, and live quota windows." })}
                </p>
            </div>
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              label={t("pages.costs.tileInference", { defaultValue: "Inference spend" })}
              value={formatCents(spendData?.summary.spendCents ?? 0)}
              subtitle={t("pages.costs.tokensAcross", { tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              label={t("pages.costs.tileBudget", { defaultValue: "Budget" })}
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("pages.costs.pausedSummary", { agents: budgetData?.pausedAgentCount ?? 0, projects: budgetData?.pausedProjectCount ?? 0 })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
            ...
            />
            <MetricTile
              label={t("pages.costs.tileFinanceNet", { defaultValue: "Finance net" })}
              value={formatCents(financeData?.summary.netCents ?? 0)}
              subtitle={t("pages.costs.debitsCredits", { debits: formatCents(financeData?.summary.debitCents ?? 0), credits: formatCents(financeData?.summary.creditCents ?? 0) })}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("pages.costs.tileFinanceEvents", { defaultValue: "Finance events" })}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={t("pages.costs.estimatedInRange", { estimated: formatCents(financeData?.summary.estimatedDebitCents ?? 0) })}
              icon={ArrowUpRight}
            />
          </div>
      </div>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-31)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    ...
                    <CardDescription>
                      {t("pages.costs.inferenceLedgerDesc", { defaultValue: "Request-scoped inference spend for the selected period." })}
                    </CardDescription>
                  </CardHeader>
                      ...
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ...
                            : t("common.unlimitedBudget", { defaultValue: "Unlimited budget" })}
                        </div>
                      </div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {t("pages.costs.utilizationInRange", { percent: spendData.summary.utilizationPercent })}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-32)">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    ...
                    <CardDescription>{t("pages.costs.byAgentDesc", { defaultValue: "What each agent consumed in the selected period." })}</CardDescription>
                  </CardHeader>
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  in {formatTokens(row.inputTokens + row.cachedInputTokens)} {t("ui.pages.costs.text")}{formatTokens(row.outputTokens)}
                                </div>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>{t("pages.costs.byProjectDesc", { defaultValue: "Run costs attributed through project-linked tasks." })}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("pages.costs.financeEmpty", { defaultValue: "No finance events yet. Add account-level charges once biller invoices or credits land." })} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-(image:--gradient-extract-2)">
                <CardHeader className="px-5 pt-5 pb-3">
                  ...
                  <CardDescription>
                    {t("pages.costs.budgetControlPlaneDesc", { defaultValue: "Hard-stop spend limits for agents and projects. Provider subscription quota stays separate and appears under Providers." })}
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    ...
                    subtitle={t("pages.costs.incidentsDesc", { defaultValue: "Open soft or hard threshold crossings" })}
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label={t("pages.costs.pendingApprovals", { defaultValue: "Pending approvals" })}
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle={t("pages.costs.approvalsDesc", { defaultValue: "Budget override approvals awaiting board action" })}
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                    label={t("pages.costs.pausedAgents", { defaultValue: "Paused agents" })}
                    value={String(budgetData?.pausedAgentCount ?? 0)}
                    subtitle={t("pages.costs.pausedAgentsDesc", { defaultValue: "Agent heartbeats blocked by budget" })}
                    icon={Coins}
                  />
                  <MetricTile
                    label={t("pages.costs.pausedProjects", { defaultValue: "Paused projects" })}
                    value={String(budgetData?.pausedProjectCount ?? 0)}
                    subtitle={t("pages.costs.pausedProjectsDesc", { defaultValue: "Project execution blocked by budget" })}
                    icon={DollarSign}
                  />
                </CardContent>
              </Card>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {t("pages.costs.incidentsHeaderDesc", { defaultValue: "Resolve hard stops here by raising the budget or explicitly keeping the scope paused." })}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">{t("pages.costs.scopeBudgets", { scope: scopeType })}</h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("ui.pages.costs.no-budget-policies-yet")}</CardContent>
                  </Card>
                ) : null}
              </div>
            </>
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-(--gtc-33)">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">{t("ui.pages.costs.biller")}</CardTitle>
                      <CardDescription>{t("ui.pages.costs.account-level-financial-events")}</CardDescription>
                    </CardHeader>
