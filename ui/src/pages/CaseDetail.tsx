...
const STATUS_LABEL: Record<CaseStatus, string> = {
  draft: t("pages.caseDetail.draft", { defaultValue: "Draft" }),
  in_progress: t("pages.caseDetail.inProgress", { defaultValue: "In progress" }),
  in_review: t("pages.caseDetail.inReview", { defaultValue: "In review" }),
  approved: t("pages.caseDetail.approved", { defaultValue: "Approved" }),
  done: t("pages.caseDetail.done", { defaultValue: "Done" }),
  cancelled: t("pages.caseDetail.cancelled", { defaultValue: "Cancelled" }),
};
...
function CaseRelationshipsSection({
  ...
  return (
    <section className="space-y-3" aria-label={t("pages.caseDetail.relationships", { defaultValue: "Case relationships" })}>
      {parent ? (
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="start" className="w-64 p-2">
        <Input
          ...
          placeholder={t("pages.caseDetail.searchLabels", { defaultValue: "Search labels…" })}
        ...
        <div className="max-h-52 space-y-0.5 overflow-y-auto">
          ...
          {filtered.length === 0 && !search.trim() && (
            <p className="px-2 py-1 text-xs text-muted-foreground">{t("pages.caseDetail.noLabels", { defaultValue: "No labels yet." })}</p>
          )}
        </div>
...
function CasePropertiesContent({
  ...
  return (
    <div className={cn("space-y-4", isFull && "space-y-6")}>
      <PropertySection title={t("pages.caseDetail.case", { defaultValue: "Case" })} first>
      ...
      {propertyRows.length > 0 ? (
        <PropertySection title={t("pages.caseDetail.fields", { defaultValue: "Fields" })}>
      ...
      {caseData.attachments.length > 0 ? (
        <PropertySection title={t("pages.caseDetail.attachments", { defaultValue: "Attachments" })}>
...
export function CaseDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("pages.caseDetail.cases", { defaultValue: "Cases" }), href: caseHref() },
      { label: caseData ? `${caseData.identifier} — ${caseData.title}` : (caseIdentifier ?? t("pages.caseDetail.case", { defaultValue: "Case" })) },
    ]);
      ...
      <header className="space-y-3">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="icon-xs" aria-label={t("pages.caseDetail.moreActions", { defaultValue: "More case actions" })} title={t("pages.caseDetail.moreActions", { defaultValue: "More case actions" })}>
