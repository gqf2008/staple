...
export function CliAuthPage() {
  ...
  if (!challengeId || !token) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">{t("pages.cliAuth.invalidUrl", { defaultValue: "Invalid CLI auth URL." })}</div>;
  }

  if (sessionQuery.isLoading || challengeQuery.isLoading) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">{t("pages.cliAuth.loading", { defaultValue: "Loading CLI auth challenge..." })}</div>;
  }
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <Card className="block p-6">
          <h1 className="text-lg font-semibold">{t("pages.cliAuth.challengeUnavailable", { defaultValue: "CLI auth challenge unavailable" })}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {challengeQuery.error instanceof Error ? challengeQuery.error.message : t("pages.cliAuth.challengeInvalid", { defaultValue: "Challenge is invalid or expired." })}
          </p>
        </Card>
      </div>
    );
  ...
  if (!challenge) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">{t("pages.cliAuth.challengeUnavailable2", { defaultValue: "CLI auth challenge unavailable." })}</div>;
  }
    ...
    <div className="mx-auto max-w-xl py-10">
      <Card className="block p-6">
        ...
        <div className="mt-5 space-y-3 text-sm">
          ...
          <div>
            ...
            <div className="text-foreground">
              {challenge.requestedAccess === "instance_admin_required" ? t("pages.cliAuth.instanceAdmin", { defaultValue: "Instance admin" }) : t("pages.cliAuth.board", { defaultValue: "Board" })}
            </div>
          </div>
        ...
        <div className="mt-5 flex gap-3">
          ...
          >
            {approveMutation.isPending ? t("pages.cliAuth.approving", { defaultValue: "Approving..." }) : t("pages.cliAuth.approveCli", { defaultValue: "Approve CLI access" })}
          </Button>
          <Button
            type="button"
            variant="outline"
