...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("pages.profileSettings.settings", { defaultValue: "Settings" }), href: "/company/settings" },
      { label: t("pages.profileSettings.instanceSettings", { defaultValue: "Instance settings" }), href: "/company/settings/instance/general" },
      { label: t("pages.profileSettings.profile", { defaultValue: "Profile" }) },
    ]);
  }, [setBreadcrumbs]);
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("pages.profileSettings.board", { defaultValue: "Board" });
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("pages.profileSettings.updateFailed", { defaultValue: "Failed to update profile." }));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("pages.profileSettings.companyFirst", { defaultValue: "Select a company before uploading a profile avatar." }));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("pages.profileSettings.uploadFailed", { defaultValue: "Failed to upload avatar." }));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("pages.profileSettings.removeFailed", { defaultValue: "Failed to remove avatar." }));
    },
  });
  ...
  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("pages.profileSettings.loadFailed", { defaultValue: "Failed to load profile." })}
      </div>
    );
  }

  const currentName = name.trim() || sessionQuery.data.user.name || t("pages.profileSettings.board", { defaultValue: "Board" });
  ...
  const uploadHint = selectedCompany
    ...
    : t("pages.profileSettings.uploadHint", { defaultValue: "Select a company to upload an avatar into Paperclip storage." });
