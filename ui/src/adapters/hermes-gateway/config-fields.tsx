...
export function HermesGatewayConfigFields({
  ...
  return (
    <>
      <Field
        label={t("components.hermesConfig.apiBaseUrl", { defaultValue: "API base URL" })}
        hint={t("ui.adapters.hermes-gateway.config-fields.hermes-api-server-base")}
      >
        <DraftInput
          value={apiBaseUrl}
      ...
      <SecretField
        label={t("components.hermesConfig.apiKey", { defaultValue: "API key" })}
        value={isCreate ? String(readCreateValue(values, "apiKey", "") ?? "") : editApiKeyValue}
        onCommit={(v) => writeValue("apiKey", v || undefined)}
        placeholder={t("components.hermesConfig.apiKeyHint", { defaultValue: "Hermes API_SERVER_KEY, not PAPERCLIP_API_KEY" })}
        stored={!isCreate && hasStoredApiKey && !editApiKeyValue}
      />

      <Field
        label={t("components.hermesConfig.paperclipApiUrl", { defaultValue: "Paperclip API URL" })}
        hint={t("components.hermesConfig.paperclipApiUrlHint", { defaultValue: "Optional Paperclip API URL reachable by the Hermes host. This is not a credential." })}
      >
        <DraftInput
      ...
      <Field
        label={t("components.hermesConfig.sessionStrategy", { defaultValue: "Session key strategy" })}
        hint={t("components.hermesConfig.sessionStrategyHint", { defaultValue: "Controls X-Hermes-Session-Key. Issue scoped prevents cross-task memory bleed by default." })}
      >
        <select
          ...
          className={inputClass}
        >
          <option value="issue">{t("components.hermesConfig.issueScoped", { defaultValue: "Issue scoped" })}</option>
          <option value="agent">{t("components.hermesConfig.agentScoped", { defaultValue: "Agent scoped" })}</option>
          <option value="run">{t("components.hermesConfig.runScoped", { defaultValue: "Run scoped" })}</option>
          <option value="none">{t("components.hermesConfig.none", { defaultValue: "None" })}</option>
        </select>
      </Field>

      <Field label={t("components.hermesConfig.timeout", { defaultValue: "Timeout seconds" })}>
        <DraftNumberInput
      ...
      <Field
        label={t("components.hermesConfig.reconnectMs", { defaultValue: "Event reconnect ms" })}
        hint={t("components.hermesConfig.reconnectMsHint", { defaultValue: "Delay before reconnecting the Hermes SSE events stream after a nonterminal disconnect." })}
      >
        <DraftNumberInput
      ...
      <ToggleField
        label={t("components.hermesConfig.allowRemoteHttp", { defaultValue: "Dangerously allow remote HTTP" })}
        hint={t("ui.adapters.hermes-gateway.config-fields.unsafe-dev-only-escape")}
        checked={allowInsecureRemoteHttp}
        onChange={(v) => writeValue("dangerouslyAllowInsecureRemoteHttp", v)}
      />

      <Field
        label={t("components.hermesConfig.extraHeaders", { defaultValue: "Extra headers" })}
        hint={t("ui.adapters.hermes-gateway.config-fields.optional-json-object-extra")}
      >
        <textarea
          value={headers}
          onChange={(event) => {
      ...
      <Field label={t("components.hermesConfig.instructions", { defaultValue: "Instructions" })} hint={t("components.hermesConfig.instructionsHint", { defaultValue: "Optional stable Hermes instructions sent separately from the wake input." })}>
        <DraftTextarea
          value={instructions}
